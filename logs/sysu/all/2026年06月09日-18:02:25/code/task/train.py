...
def _common_hard_negative_loss(
    features, positive_ids, target_memory, candidate_ids, margin, topk, require_positive_top1
):
    empty_stats = {
        'samples': 0,
        'positive_similarity': features.new_tensor(0.0),
        'negative_similarity': features.new_tensor(0.0),
        'gap': features.new_tensor(0.0),
        'eligible_ratio': features.new_tensor(0.0),
        'active_ratio': features.new_tensor(0.0),
    }
    if features.shape[0] == 0 or candidate_ids.shape[0] < 2:
        return features.new_tensor(0.0), empty_stats

    features = F.normalize(features, dim=1)
    candidates = F.normalize(target_memory[candidate_ids].detach(), dim=1)
    positive_memory = F.normalize(target_memory[positive_ids].detach(), dim=1)
    positive_similarity = (features * positive_memory).sum(dim=1)

    similarities = torch.matmul(features, candidates.t())
    positive_mask = candidate_ids.unsqueeze(0).eq(positive_ids.unsqueeze(1))
    similarities = similarities.masked_fill(positive_mask, float('-inf'))
    hard_negative_num = min(max(1, topk), candidate_ids.shape[0] - 1)
    hard_negative_values = similarities.topk(hard_negative_num, dim=1).values
    negative_similarity = hard_negative_values.mean(dim=1)
    if require_positive_top1:
        eligible = positive_similarity.detach() >= hard_negative_values[:, 0].detach()
    else:
        eligible = torch.ones_like(positive_similarity, dtype=torch.bool)

    violations = margin + negative_similarity - positive_similarity
    eligible_count = eligible.float().sum().clamp_min(1.0)
    loss = (F.relu(violations) * eligible.float()).sum() / eligible_count
    stats = {
        'samples': features.shape[0],
        'positive_similarity': positive_similarity.detach().mean(),
        'negative_similarity': negative_similarity.detach().mean(),
        'gap': (positive_similarity - negative_similarity).detach().mean(),
        'eligible_ratio': eligible.float().mean(),
        'active_ratio': ((violations.detach() > 0) & eligible).float().sum() / eligible_count,
    }
    return loss, stats
        ...
        if 'wsl' in args.debug:
            if enable_phase1:
                ...
                if args.debug == 'wsl':
                    ...
                    cmhn_weight = getattr(args, 'cmhn_weight', 0.0)
                    cmhn_start_epoch = getattr(args, 'cmhn_start_epoch', 10)
                    cmhn_end_epoch = getattr(args, 'cmhn_end_epoch', 70)
                    if cmhn_weight > 0 and cmhn_start_epoch <= epoch < cmhn_end_epoch:
                        cmhn_direction_losses = []
                        cmhn_direction_stats = []
                        cmhn_margin = getattr(args, 'cmhn_margin', 0.2)
                        cmhn_topk = getattr(args, 'cmhn_topk', 1)
                        cmhn_require_positive_top1 = bool(getattr(args, 'cmhn_require_positive_top1', 1))

                        if selected_common_rgb_ids.shape[0] != 0:
                            rgb_cmhn_loss, rgb_cmhn_stats = _common_hard_negative_loss(
                                ...
                                cmhn_margin,
                                cmhn_topk,
                                cmhn_require_positive_top1,
                            )
                            if rgb_cmhn_stats['samples'] > 0:
                                cmhn_direction_losses.append(rgb_cmhn_loss)
                                cmhn_direction_stats.append(rgb_cmhn_stats)

                        if selected_common_ir_ids.shape[0] != 0:
                            ir_cmhn_loss, ir_cmhn_stats = _common_hard_negative_loss(
                            ...
                            if ir_cmhn_stats['samples'] > 0:
                                cmhn_direction_losses.append(ir_cmhn_loss)
                                cmhn_direction_stats.append(ir_cmhn_stats)

                        if len(cmhn_direction_losses) > 0:
                            cmhn_loss = cmhn_weight * sum(cmhn_direction_losses) / len(cmhn_direction_losses)
                            if torch.isnan(cmhn_loss).any():
                                nan_batch_counter += 1
                            else:
                                cmhn_samples = sum(stats['samples'] for stats in cmhn_direction_stats)
                                cmhn_metric = lambda key: sum(
                                    stats[key] * stats['samples'] for stats in cmhn_direction_stats
                                ) / cmhn_samples
                                meter.update({
                                    'cmhn_loss': cmhn_loss.data,
                                    'cmhn_gap': cmhn_metric('gap'),
                                    'cmhn_eligible_ratio': cmhn_metric('eligible_ratio'),
                                    'cmhn_active_ratio': cmhn_metric('active_ratio'),
                                    'cmhn_positive_similarity': cmhn_metric('positive_similarity'),
                                    'cmhn_negative_similarity': cmhn_metric('negative_similarity'),
                                    'cmhn_samples': cmhn_samples,
                                })
                                total_loss += cmhn_loss
