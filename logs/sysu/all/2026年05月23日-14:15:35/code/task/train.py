    ...
    if 'wsl' in args.debug or not enable_phase1:
        ...
        for i,r in i2r_pair_dict.items():
            if (r,i) in common_dict.items():
                continue
            elif r not in r2i_pair_dict.keys() and i not in r2i_pair_dict.values():
        ...
        if getattr(args, 'enable_trrm', 1):
            ...
            ramp_epochs = max(1, getattr(args, 'trrm_specific_ramp', 10))
            ...
            logger(
                'TRRM diag | specific_kept:{}/{} active:{} avg_mem:{:.4f} avg_streak:{:.2f} specific_w:{:.3f}'.format(
                    len(specific_dict),
                    raw_specific_num,
                    memory_stats['active'],
            ...
            if enable_phase1:
                ...
                if epoch >= remain_start_epoch:
                    ...
                    if (remain_rgb_indices.shape[0]>0):
                        weak_r2c_loss = remain_weight * args.weak_weight*model.weak_criterion(remain_r2c_cls, remain_rm[remain_rgb_ids])
