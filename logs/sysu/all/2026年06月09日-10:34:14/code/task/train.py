...
def train(args, model: Model, dataset, *arg):
    ...
    if 'wsl' in args.debug or not enable_phase1:
        ...
        specific_only_rm = specific_rm.clone()
            ...
            if enable_phase1:
                ...
                if args.debug == 'wsl':
                    ...
                    if rgmfd_pack is not None and epoch >= getattr(args, 'rgmfd_start_epoch', 0):
                        ...
                        specific_shared_weight = getattr(args, 'rshd_specific_shared_weight', 0.0)
                        if specific_shared_weight > 0:
                            specific_rel_losses = []
                            specific_rgb_indices = torch.isin(rgb_ids, specific_matched_rgb)
                            specific_ir_indices = torch.isin(ir_ids, specific_matched_ir)
                            selected_specific_rgb_ids = rgb_ids[specific_rgb_indices]
                            selected_specific_ir_ids = ir_ids[specific_ir_indices]

                            if selected_specific_rgb_ids.shape[0] != 0:
                                translated_specific_rgb_label = torch.nonzero(
                                    specific_only_rm[selected_specific_rgb_ids]
                                )[:, -1]
                                selected_specific_rgb_shared = shared_bn[:2*bt][specific_rgb_indices]
                                selected_specific_ir_memory = cma.ir_memory[
                                    translated_specific_rgb_label
                                ].detach()
                                specific_rel_losses.append(
                                    _cosine_alignment_loss(
                                        selected_specific_rgb_shared,
                                        selected_specific_ir_memory,
                                    )
                                )

                            if selected_specific_ir_ids.shape[0] != 0:
                                translated_specific_ir_label = torch.nonzero(
                                    specific_only_rm.T[selected_specific_ir_ids]
                                )[:, -1]
                                selected_specific_ir_shared = shared_bn[2*bt:][specific_ir_indices]
                                selected_specific_rgb_memory = cma.vis_memory[
                                    translated_specific_ir_label
                                ].detach()
                                specific_rel_losses.append(
                                    _cosine_alignment_loss(
                                        selected_specific_ir_shared,
                                        selected_specific_rgb_memory,
                                    )
                                )

                            if len(specific_rel_losses) > 0:
                                rshd_specific_shared_loss = (
                                    specific_shared_weight
                                    * sum(specific_rel_losses) / len(specific_rel_losses)
                                )
                                if torch.isnan(rshd_specific_shared_loss).any():
                                    nan_batch_counter += 1
                                else:
                                    meter.update({
                                        'rshd_specific_shared_loss': rshd_specific_shared_loss.data,
                                        'rshd_specific_samples': (
                                            selected_specific_rgb_ids.shape[0]
                                            + selected_specific_ir_ids.shape[0]
                                        ),
                                    })
                                    total_loss += rshd_specific_shared_loss
