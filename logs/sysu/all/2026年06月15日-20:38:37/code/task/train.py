...
from models.relation import build_uprt_posterior, posterior_cross_modal_loss
...
def train(args, model: Model, dataset, *arg):
    ...
    if args.debug == 'wsl':
        ...
        if getattr(args, 'enable_uprt', 0) and not enable_phase1:
            uprt_state = build_uprt_posterior(args, cma, common_dict)
            ...
            logger(
                ...
                'entropy:{:.3f} common_p:{:.3f} cand:{:.2f}%'.format(
                    int(uprt_scale > 0),
                    uprt_scale,
