...
def _cross_modal_prototype_ranking_loss(
    query_features,
    source_prototype_memory,
    source_indices,
    target_prototype_memory,
    positive_indices,
    args,
):
    if query_features.shape[0] == 0:
        ...
        return zero, {
            'samples': 0,
            'query_topk_ratio': zero.detach(),
            'mutual_topk_ratio': zero.detach(),
            'reliable_ratio': zero.detach(),
    ...
    min_positive_similarity = getattr(args, 'cmpc_min_positive_similarity', 0.2)
    topk = max(1, getattr(args, 'cmpc_topk', 1))
    target_margin = getattr(args, 'cmpc_margin', 0.18)
    source_prototype_norm = source_prototype_memory.detach().norm(dim=1)
    target_prototype_norm = target_prototype_memory.detach().norm(dim=1)
    valid_source_prototypes = source_prototype_norm > 1e-12
    valid_target_prototypes = target_prototype_norm > 1e-12

    query_features = F.normalize(query_features, dim=1)
    source_prototypes = F.normalize(source_prototype_memory.detach(), dim=1)
    target_prototypes = F.normalize(target_prototype_memory.detach(), dim=1)
    similarities = query_features @ target_prototypes.t()
    positive_similarity = similarities.gather(1, positive_indices[:, None]).squeeze(1)

    ranked_similarities = similarities.detach().masked_fill(~valid_target_prototypes[None, :], -1e4)
    positive_rank = (
        ranked_similarities > positive_similarity.detach()[:, None]
    ).sum(dim=1) + 1
    query_topk = positive_rank <= topk

    positive_prototypes = target_prototypes[positive_indices]
    reverse_similarities = positive_prototypes @ source_prototypes.t()
    reverse_similarities = reverse_similarities.masked_fill(
        ~valid_source_prototypes[None, :], -1e4
    )
    reverse_positive_similarity = reverse_similarities.gather(
        1, source_indices[:, None]
    ).squeeze(1)
    reverse_positive_rank = (
        reverse_similarities > reverse_positive_similarity[:, None]
    ).sum(dim=1) + 1
    mutual_topk = reverse_positive_rank <= topk

    reliable = (
        valid_source_prototypes[source_indices]
        & valid_target_prototypes[positive_indices]
        & (positive_similarity.detach() >= min_positive_similarity)
        & query_topk
        & mutual_topk
    )

    negative_similarities = similarities.masked_fill(~valid_target_prototypes[None, :], -1e4)
    negative_similarities.scatter_(1, positive_indices[:, None], -1e4)
    hard_negative_similarity = negative_similarities.max(dim=1).values
    margin_gap = positive_similarity - hard_negative_similarity
    per_sample_loss = F.relu(target_margin - margin_gap)
    reliable_count = reliable.float().sum().clamp_min(1.0)
    loss = (per_sample_loss * reliable.float()).sum() / reliable_count
    violation = reliable & (per_sample_loss.detach() > 0)

    stats = {
        'samples': query_features.shape[0],
        'query_topk_ratio': query_topk.float().mean(),
        'mutual_topk_ratio': mutual_topk.float().mean(),
        'reliable_ratio': reliable.float().mean(),
        'violation_ratio': violation.float().sum() / reliable_count,
        'positive_similarity': positive_similarity.detach().mean(),
        'hard_negative_similarity': hard_negative_similarity.detach().mean(),
        'margin': margin_gap.detach().mean(),
    }
    return loss, stats
        ...
        if 'wsl' in args.debug:
            if enable_phase1:
                ...
                if args.debug == 'wsl':
                    ...
                    entropy_sum = (r2i_entropy + i2r_entropy).clamp_min(1e-12)
                    w_r2i = r2i_entropy / entropy_sum
                    w_i2r = i2r_entropy / entropy_sum
                    ...
                    if selected_common_ir_ids.shape[0] != 0:
                        r2i_cmo_loss = (
                            common_loss_weight
                            * w_r2i
                            * cmo_criterion(dtd_i2i_cls[common_ir_indices], mem_r2i_cls)
                        )
                    ...
                    if selected_common_rgb_ids.shape[0] != 0:
                        i2r_cmo_loss = (
                            common_loss_weight
                            * w_i2r
                            * cmo_criterion(dtd_r2r_cls[common_rgb_indices], mem_i2r_cls)
                        )
                    ...
                    cmpc_start_epoch = getattr(args, 'cmpc_start_epoch', 70)
                    cmpc_end_epoch = getattr(args, 'cmpc_end_epoch', 90)
                    cmpc_base_weight = getattr(args, 'cmpc_weight', 0.0)
                    if cmpc_start_epoch <= epoch < cmpc_end_epoch and cmpc_base_weight > 0:
                        cmpc_warmup_epochs = max(1, getattr(args, 'cmpc_warmup_epochs', 10))
                        cmpc_ramp = min(1.0, (epoch - cmpc_start_epoch + 1) / cmpc_warmup_epochs)
                        cmpc_weight = common_loss_weight * cmpc_base_weight * cmpc_ramp
                        cmpc_stats = []
                        if selected_common_rgb_ids.shape[0] != 0:
                            rgb_cmpc_loss, rgb_cmpc_stats = _cross_modal_prototype_ranking_loss(
                                bn_features[:2*bt][common_rgb_indices],
                                cma.vis_memory,
                            ...
                            rgb_cmpc_loss = cmpc_weight * rgb_cmpc_loss
                            if not torch.isnan(rgb_cmpc_loss).any():
                                meter.update({'rgb_cmpc_loss': rgb_cmpc_loss.data})
                                total_loss += rgb_cmpc_loss
                                cmpc_stats.append(rgb_cmpc_stats)
                        if selected_common_ir_ids.shape[0] != 0:
                            ir_cmpc_loss, ir_cmpc_stats = _cross_modal_prototype_ranking_loss(
                                bn_features[2*bt:][common_ir_indices],
                                cma.ir_memory,
                            ...
                            ir_cmpc_loss = cmpc_weight * ir_cmpc_loss
                            if not torch.isnan(ir_cmpc_loss).any():
                                meter.update({'ir_cmpc_loss': ir_cmpc_loss.data})
                                total_loss += ir_cmpc_loss
                                cmpc_stats.append(ir_cmpc_stats)

                        if cmpc_stats:
                            sample_num = sum(stats['samples'] for stats in cmpc_stats)
                            weighted_stat = lambda key: sum(
                                stats[key] * stats['samples'] for stats in cmpc_stats
                            ) / sample_num
                            meter.update({
                                'cmpc_query_topk_ratio': weighted_stat('query_topk_ratio'),
                                'cmpc_mutual_topk_ratio': weighted_stat('mutual_topk_ratio'),
                                'cmpc_reliable_ratio': weighted_stat('reliable_ratio'),
                                'cmpc_violation_ratio': weighted_stat('violation_ratio'),
                                'cmpc_positive_sim': weighted_stat('positive_similarity'),
                                'cmpc_hard_negative_sim': weighted_stat('hard_negative_similarity'),
                                'cmpc_margin': weighted_stat('margin'),
                                'cmpc_ramp': cmpc_ramp,
                            })
