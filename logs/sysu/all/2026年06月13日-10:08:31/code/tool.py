...
def infer_method(args: dict[str, Any]) -> str:
    if value_is_enabled(args.get("enable_rgmfd")):
        if value_is_positive(args.get("cmpc_weight")):
            return "RG-MFD+CMPC"
        return "RG-MFD+RDL" if value_is_enabled(args.get("enable_rdl")) else "RG-MFD"
    if value_is_enabled(args.get("enable_trrm")):
        return "TRRM"
    if value_is_enabled(args.get("enable_remain_gate")):
        return "RemainGate"
    return "Base/CRE"
...
def build_short_params(args: dict[str, Any]) -> str:
    pairs = [
        ...
        ("cmpc_weight", "cmpc_w", None),
        ("cmpc_margin", "cmpc_margin", None),
        ("cmpc_topk", "cmpc_topk", None),
        ("cmpc_min_positive_similarity", "cmpc_min_sim", None),
        ("cmpc_start_epoch", "cmpc_start", None),
        ("cmpc_end_epoch", "cmpc_end", None),
        ("cmpc_warmup_epochs", "cmpc_warmup", None),
        ("enable_rdl", "rdl", "-"),
        ("enable_trrm", "trrm", "-"),
        ("enable_remain_gate", "remain_gate", "-"),
    ]
