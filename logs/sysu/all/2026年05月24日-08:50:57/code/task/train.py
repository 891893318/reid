        ...
        if 'wsl' in args.debug:
            if enable_phase1:
                ...
                if args.debug == 'wsl':
                    ...
                    # 混合特征送入计算跨模态的三元组误差距离
                    common_tri_mult = getattr(args, 'common_tri_weight', 1.0)
                    tri_loss_rgb = args.tri_weight * model.tri_criterion(matched_tri_rgb_features, matched_tri_rgb_labels) * common_tri_mult
                    tri_loss_ir = args.tri_weight * model.tri_criterion(matched_tri_ir_features, matched_tri_ir_labels) * common_tri_mult
                    ...
                    # 验证并叠加 CMO 损失
                    common_cmo_mult = getattr(args, 'common_cmo_weight', 1.0)
                    if (selected_tri_ir_ids.shape[0]!=0):
                        r2i_cmo_loss = w_r2i * cmo_criterion(dtd_i2i_cls[common_ir_indices],mem_r2i_cls) * common_cmo_mult
                    ...
                    if (selected_tri_rgb_ids.shape[0]!=0):
                        i2r_cmo_loss = w_i2r * cmo_criterion(dtd_r2r_cls[common_rgb_indices],mem_i2r_cls) * common_cmo_mult
        ...
        if enable_phase1:
            ...
            if args.debug == 'wsl':
                ...
                if (selected_rgb_ids.shape[0]>0):
                    specific_weight = specific_loss_weight if getattr(args, 'enable_trrm', 1) else 1.0
