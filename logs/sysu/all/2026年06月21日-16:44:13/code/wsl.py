...
class CMA(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.register_buffer(
            'uprt_vis_memory', torch.zeros(self.num_classes, 2048, device=self.device)
        )
        self.register_buffer(
            'uprt_ir_memory', torch.zeros(self.num_classes, 2048, device=self.device)
        )
        self.register_buffer(
            'uprt_v2t', torch.full((self.num_classes, self.num_classes), 1.0 / self.num_classes, device=self.device)
        )
        self.register_buffer(
            'uprt_t2v', torch.full((self.num_classes, self.num_classes), 1.0 / self.num_classes, device=self.device)
        )
    ...
    def extract(self, args, model, dataset, collect_uprt=False):
        '''
        ...
        with torch.no_grad():
            ...
            rgb_features, rgb_labels, rgb_gt, r2i_cls, rgb_idx, rgb_uprt_memory, rgb_uprt_probs = \
                self._extract_feature(model, rgb_loader,'rgb', collect_uprt)
            ir_features, ir_labels, ir_gt, i2r_cls, ir_idx, ir_uprt_memory, ir_uprt_probs = \
                self._extract_feature(model, ir_loader,'ir', collect_uprt)
        ...
        if collect_uprt:
            self.uprt_vis_memory = rgb_uprt_memory.detach()
            self.uprt_ir_memory = ir_uprt_memory.detach()
            self.uprt_v2t = rgb_uprt_probs.detach()
            self.uprt_t2v = ir_uprt_probs.detach()
        
    def _extract_feature(self, model, loader, modal, collect_uprt=False):

        print('extracting {} features'.format(modal))

        saved_features, saved_labels, saved_cls= None, None, None
        saved_gts, saved_idx= None, None
        uprt_feature_sums = None
        uprt_prob_sums = None
        uprt_counts = None
        if collect_uprt:
            uprt_feature_sums = torch.zeros(self.num_classes, 2048, device=model.device)
            uprt_prob_sums = torch.zeros(self.num_classes, self.num_classes, device=model.device)
            uprt_counts = torch.zeros(self.num_classes, device=model.device)
        for imgs_list, infos in loader:
            ...
            if imgs_list.__class__.__name__ != 'list':
                ...
                imgs, labels, idx= \
                    imgs.to(model.device), labels.to(model.device), idx.to(model.device)
            _, bn_features = model.model(imgs) # Keep the existing CRE extraction path.
            if modal == 'rgb':
            ...
            if collect_uprt:
                if modal == 'rgb':
                    uprt_features = bn_features
                    uprt_cls = cls
                elif modal == 'ir':
                    _, uprt_features = model.model(x2=imgs)
                    uprt_cls, _ = model.classifier1(uprt_features)
                uprt_probs = torch.nn.functional.softmax(self.T * uprt_cls, dim=1)
                uprt_feature_sums.index_add_(0, labels, uprt_features)
                uprt_prob_sums.index_add_(0, labels, uprt_probs)
                uprt_counts.index_add_(0, labels, torch.ones_like(labels, dtype=uprt_counts.dtype))
        ...
        if collect_uprt:
            uprt_counts = uprt_counts.clamp_min(1.0).unsqueeze(1)
            uprt_memory = uprt_feature_sums / uprt_counts
            uprt_probs = uprt_prob_sums / uprt_counts
        else:
            uprt_memory = None
            uprt_probs = None
        return (
