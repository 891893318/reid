...
def infer_method(args: dict[str, Any]) -> str:
    if value_is_enabled(args.get("enable_rgmfd")):
        if value_is_positive(args.get("cmhn_weight")):
            return "RG-MFD+CMHN"
...
def build_short_params(args: dict[str, Any]) -> str:
    pairs = [
        ...
        ("cmhn_weight", "cmhn_w", None),
        ("cmhn_margin", "cmhn_m", None),
        ("cmhn_start_epoch", "cmhn_start", None),
        ("cmhn_end_epoch", "cmhn_end", None),
        ("cmhn_require_positive_top1", "cmhn_gate", None),
        ("enable_rdl", "rdl", "-"),
        ("enable_trrm", "trrm", "-"),
