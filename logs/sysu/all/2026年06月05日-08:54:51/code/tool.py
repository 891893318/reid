...
def infer_method(args: dict[str, Any]) -> str:
    ...
    if value_is_enabled(args.get("enable_rgmfd")):
        parts.append("RG-MFD")
    if value_is_enabled(args.get("enable_ddclae")):
        parts.append("DD-CLAE")
    if value_is_enabled(args.get("enable_rdl")):
        parts.append("RDL")
    if parts:
        return "+".join(parts)
...
def build_short_params(args: dict[str, Any]) -> str:
    pairs = [
        ...
        ("enable_ddclae", "ddclae", "-"),
        ("ddclae_start_epoch", "dd_s", None),
        ("ddclae_weight", "dd_w", None),
        ("ddclae_temperature", "dd_t", None),
        ("ddclae_mid", "dd_mid", None),
        ("ddclae_bandwidth", "dd_bw", None),
        ("enable_rdl", "rdl", "-"),
        ("enable_trrm", "trrm", "-"),
