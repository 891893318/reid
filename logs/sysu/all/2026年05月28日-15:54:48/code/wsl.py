...
class CMA(nn.Module):
    ...
    def _eot_partition_relations(self, r2i_pair_dict, i2r_pair_dict, reliability, args):
        common_threshold = getattr(args, 'eot_common_threshold', 0.08)
        specific_threshold = getattr(args, 'eot_specific_threshold', 0.04)
        remain_threshold = getattr(args, 'eot_remain_threshold', 0.02)
        ...
        for r, i in r2i_pair_dict.items():
            ...
            if i in i2r_pair_dict and i2r_pair_dict[i] == r and rel >= common_threshold:
                common_dict[r] = i
            elif rel >= specific_threshold and r not in i2r_pair_dict.values() and i not in i2r_pair_dict.keys():
                specific_dict[r] = i
            elif rel >= remain_threshold:
                remain_dict[r] = i

        for i, r in i2r_pair_dict.items():
            if r in common_dict and common_dict[r] == i:
                continue
            rel = reliability[int(r), int(i)]
            if rel >= specific_threshold and r not in r2i_pair_dict.keys() and i not in r2i_pair_dict.values():
                specific_dict[r] = i
            elif rel >= remain_threshold:
                remain_dict[r] = i
