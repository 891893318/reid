...
def _uncertainty_gated_clae_loss(native_logits, cross_logits, native_labels, args):
    if native_logits.shape[0] == 0:
        zero = native_logits.new_tensor(0.0)
        return zero, {
            'samples': 0,
            'gate': zero,
            'conflict': zero,
            ...
            'student_confidence': zero,
            'teacher_accuracy': zero,
        }

    temperature = max(getattr(args, 'uclae_temperature', 1.0), 1e-6)
    conflict_temperature = max(getattr(args, 'uclae_conflict_temperature', 0.25), 1e-6)
    min_teacher_confidence = getattr(args, 'uclae_min_teacher_confidence', 0.0)
    require_native_correct = bool(getattr(args, 'uclae_require_native_correct', 1))

    teacher_probs = F.softmax(native_logits.detach() / temperature, dim=1)
    student_log_probs = F.log_softmax(cross_logits / temperature, dim=1)
    student_probs = student_log_probs.detach().exp()
    class_entropy = float(np.log(max(teacher_probs.shape[1], 2)))

    teacher_entropy = -(teacher_probs * teacher_probs.clamp_min(1e-12).log()).sum(dim=1) / class_entropy
    student_entropy = -(student_probs * student_probs.clamp_min(1e-12).log()).sum(dim=1) / class_entropy
    teacher_confidence = (1.0 - teacher_entropy).clamp(0.0, 1.0)
    student_confidence = (1.0 - student_entropy).clamp(0.0, 1.0)

    mixture = 0.5 * (teacher_probs + student_probs)
    teacher_kl = (
        teacher_probs * (teacher_probs.clamp_min(1e-12).log() - mixture.clamp_min(1e-12).log())
    ).sum(dim=1)
    student_kl = (
        student_probs * (student_probs.clamp_min(1e-12).log() - mixture.clamp_min(1e-12).log())
    ).sum(dim=1)
    conflict = (0.5 * (teacher_kl + student_kl) / np.log(2.0)).clamp(0.0, 1.0)

    active = teacher_confidence >= min_teacher_confidence
    teacher_correct = teacher_probs.argmax(dim=1).eq(native_labels)
    if require_native_correct:
        active = active & teacher_correct

    gate = teacher_confidence * torch.exp(-conflict / conflict_temperature)
    gate = gate * active.float()
    per_sample_kl = F.kl_div(student_log_probs, teacher_probs, reduction='none').sum(dim=1)
    loss = (temperature ** 2) * (gate * per_sample_kl).mean()
    stats = {
        'samples': native_logits.shape[0],
        'gate': gate.detach().mean(),
        'conflict': conflict.detach().mean(),
        'active_ratio': active.detach().float().mean(),
        'teacher_confidence': teacher_confidence.detach().mean(),
        'student_confidence': student_confidence.detach().mean(),
        'teacher_accuracy': teacher_correct.detach().float().mean(),
    }
    return loss, stats
        ...
        if 'wsl' in args.debug:
            if enable_phase1:
                ...
                if args.debug == 'wsl':
                    ...
                    translated_uclae_rgb_label = torch.nonzero(common_rm[selected_common_rgb_ids])[:,-1]
                    translated_uclae_ir_label = torch.nonzero(common_rm.T[selected_common_ir_ids])[:,-1]
                    ...
                    selected_rgb_memory = cma.vis_memory[translated_uclae_ir_label].detach()
                    selected_ir_memory = cma.ir_memory[translated_uclae_rgb_label].detach()
                    
                    # 取分类器对该原型的分布响应
                    mem_r2i_cls,_ = model.classifier2(selected_rgb_memory)
                    mem_i2r_cls,_ = model.classifier1(selected_ir_memory)

                    # 由本模态可靠预测单向指导跨模态原型，冲突或不确定样本自动降权
                    uclae_stats = []
                    uclae_weight = getattr(args, 'uclae_weight', 1.0)
                    if selected_common_ir_ids.shape[0] != 0:
                        r2i_uclae_loss, r2i_uclae_stats = _uncertainty_gated_clae_loss(
                            dtd_i2i_cls[common_ir_indices],
                            mem_r2i_cls,
                            selected_common_ir_ids,
                            args,
                        )
                        r2i_uclae_loss = common_loss_weight * uclae_weight * r2i_uclae_loss
                        if torch.isnan(r2i_uclae_loss).any():
                            nan_batch_counter+=1
                        else:
                            meter.update({'r2i_uclae_loss':r2i_uclae_loss.data})
                            total_loss += r2i_uclae_loss
                            uclae_stats.append(r2i_uclae_stats)
                    if selected_common_rgb_ids.shape[0] != 0:
                        i2r_uclae_loss, i2r_uclae_stats = _uncertainty_gated_clae_loss(
                            dtd_r2r_cls[common_rgb_indices],
                            mem_i2r_cls,
                            selected_common_rgb_ids,
                            args,
                        )
                        i2r_uclae_loss = common_loss_weight * uclae_weight * i2r_uclae_loss
                        if torch.isnan(i2r_uclae_loss).any():
                            nan_batch_counter+=1
                        else:
                            meter.update({'i2r_uclae_loss':i2r_uclae_loss.data})
                            total_loss += i2r_uclae_loss
                            uclae_stats.append(i2r_uclae_stats)

                    if uclae_stats:
                        sample_num = sum(stats['samples'] for stats in uclae_stats)
                        weighted_stat = lambda key: sum(
                            stats[key] * stats['samples'] for stats in uclae_stats
                        ) / sample_num
                        meter.update({
                            'uclae_gate': weighted_stat('gate'),
                            'uclae_conflict': weighted_stat('conflict'),
                            'uclae_active_ratio': weighted_stat('active_ratio'),
                            'uclae_teacher_conf': weighted_stat('teacher_confidence'),
                            'uclae_student_conf': weighted_stat('student_confidence'),
                            'uclae_teacher_acc': weighted_stat('teacher_accuracy'),
                        })
