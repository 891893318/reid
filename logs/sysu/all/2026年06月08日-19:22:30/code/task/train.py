...
def _cross_modal_hard_ranking_loss(anchor_features, positive_memory, memory_bank,
                                   positive_labels, margin, topk, pos_weight):
    if anchor_features.numel() == 0 or positive_memory.numel() == 0 or memory_bank.numel() == 0:
        return anchor_features.new_tensor(0.0)
    if memory_bank.shape[0] <= 1:
        return anchor_features.new_tensor(0.0)

    positive_labels = positive_labels.long()
    valid = (positive_labels >= 0) & (positive_labels < memory_bank.shape[0])
    if valid.sum() == 0:
        return anchor_features.new_tensor(0.0)

    anchor = F.normalize(anchor_features[valid], dim=1)
    positive = F.normalize(positive_memory[valid].detach(), dim=1)
    memory = F.normalize(memory_bank.detach(), dim=1)

    pos_sim = (anchor * positive).sum(dim=1)
    sim = torch.matmul(anchor, memory.t())
    sim.scatter_(1, positive_labels[valid].view(-1, 1), -1e4)

    k = min(max(1, int(topk)), sim.shape[1] - 1)
    hard_neg = sim.topk(k=k, dim=1).values
    hard_neg = torch.logsumexp(hard_neg / 0.1, dim=1) * 0.1

    neg_loss = F.softplus(hard_neg - pos_sim + margin).mean()
    pos_loss = (1.0 - pos_sim).clamp_min(0.0).mean()
    return neg_loss + pos_weight * pos_loss
...
def train(args, model: Model, dataset, *arg):
    ...
    if 'wsl' in args.debug or not enable_phase1:
        ...
        specific_matched_rgb = list(specific_dict.keys())
            ...
            if enable_phase1:
                ...
                if args.debug == 'wsl':
                    ...
                    if rgmfd_pack is not None and epoch >= getattr(args, 'rgmfd_start_epoch', 0):
                        ...
                        if len(rel_losses) > 0:
                            rgmfd_rel_loss = getattr(args, 'rgmfd_rel_weight', 0.0) * sum(rel_losses) / len(rel_losses)
                            if not torch.isnan(rgmfd_rel_loss).any():
                                meter.update({'rgmfd_rel_loss': rgmfd_rel_loss.data})
                                total_loss += rgmfd_rel_loss

                    rank_hard_weight = getattr(args, 'rank_hard_weight', 0.0)
                    if epoch >= getattr(args, 'rank_hard_start', 30) and rank_hard_weight > 0:
                        rank_losses = []
                        rank_features = rgmfd_pack['shared_bn'] if rgmfd_pack is not None else bn_features
                        if selected_common_rgb_ids.shape[0] != 0:
                            rank_rgb_features = rank_features[:2*bt][common_rgb_indices]
                            rank_losses.append(
                                _cross_modal_hard_ranking_loss(
                                    rank_rgb_features,
                                    selected_ir_memory,
                                    cma.ir_memory,
                                    translated_cmo_rgb_label,
                                    getattr(args, 'rank_hard_margin', 0.1),
                                    getattr(args, 'rank_hard_topk', 5),
                                    getattr(args, 'rank_pos_weight', 0.2),
                                )
                            )
                        if selected_common_ir_ids.shape[0] != 0:
                            rank_ir_features = rank_features[2*bt:][common_ir_indices]
                            rank_losses.append(
                                _cross_modal_hard_ranking_loss(
                                    rank_ir_features,
                                    selected_rgb_memory,
                        ...
                        if len(rank_losses) > 0:
                            rank_hard_loss = rank_hard_weight * sum(rank_losses) / len(rank_losses)
                            if not torch.isnan(rank_hard_loss).any():
                                meter.update({'rank_hard_loss': rank_hard_loss.data})
                                total_loss += rank_hard_loss

                # ================ C. 中后期处理模糊(remain)弱监督边界 ================
                remain_start_epoch = 30
