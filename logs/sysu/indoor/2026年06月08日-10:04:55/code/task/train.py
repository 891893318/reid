...
def _hard_negative_prototype_loss(
    features, positive_labels, prototype_memory, margin, min_separation=0.0
):
    zero = features.sum() * 0.0
    empty_stats = {
        'margin': features.new_tensor(0.0),
        'active': features.new_tensor(0.0),
        'reliable': features.new_tensor(0.0),
        'valid': features.new_tensor(0.0),
    }
    if features.numel() == 0 or positive_labels.numel() == 0:
        return zero, empty_stats

    memory = prototype_memory.detach()
    valid_memory = memory.norm(dim=1) > 1e-12
    if valid_memory.sum().item() <= 1:
        return zero, empty_stats

    positive_labels = positive_labels.long()
    valid_rows = valid_memory[positive_labels]
    if not valid_rows.any():
        return zero, empty_stats

    features = F.normalize(features[valid_rows], dim=1)
    positive_labels = positive_labels[valid_rows]
    memory = F.normalize(memory, dim=1)
    similarities = features @ memory.T
    positive_similarity = similarities.gather(1, positive_labels.unsqueeze(1)).squeeze(1)

    negative_mask = valid_memory.unsqueeze(0).expand_as(similarities).clone()
    negative_mask.scatter_(1, positive_labels.unsqueeze(1), False)
    hard_negative_similarity = similarities.masked_fill(
        ~negative_mask, float('-inf')
    ).max(dim=1).values

    separation_margin = positive_similarity - hard_negative_similarity
    reliable_mask = separation_margin > min_separation
    violations = torch.where(
        reliable_mask,
        F.relu(margin - separation_margin),
        torch.zeros_like(separation_margin),
    )
    stats = {
        'margin': separation_margin.detach().mean(),
        'active': (violations.detach() > 0).float().mean(),
        'reliable': reliable_mask.detach().float().mean(),
        'valid': features.new_tensor(float(features.shape[0])),
    }
    return violations.mean(), stats
        ...
        if 'wsl' in args.debug:
            if enable_phase1:
                ...
                if args.debug == 'wsl':
                    ...
                    if (
                        getattr(args, 'enable_hnp', 0)
                        and rgmfd_pack is not None
                        and epoch >= getattr(args, 'hnp_start_epoch', 30)
                    ):
                        hnp_losses = []
                        hnp_stats = []
                        shared_bn = rgmfd_pack['shared_bn']
                        hnp_margin = getattr(args, 'hnp_margin', 0.1)
                        hnp_min_separation = getattr(args, 'hnp_min_separation', 0.0)
                        if selected_common_rgb_ids.shape[0] != 0:
                            rgb_hnp_loss, rgb_hnp_stats = _hard_negative_prototype_loss(
                                shared_bn[:2*bt][common_rgb_indices],
                                translated_cmo_rgb_label,
                                cma.ir_memory,
                                hnp_margin,
                                hnp_min_separation,
                            )
                            hnp_losses.append(rgb_hnp_loss)
                            hnp_stats.append(rgb_hnp_stats)
                        if selected_common_ir_ids.shape[0] != 0:
                            ir_hnp_loss, ir_hnp_stats = _hard_negative_prototype_loss(
                                shared_bn[2*bt:][common_ir_indices],
                                translated_cmo_ir_label,
                                cma.vis_memory,
                                hnp_margin,
                                hnp_min_separation,
                            )
                            hnp_losses.append(ir_hnp_loss)
                            hnp_stats.append(ir_hnp_stats)
                        if len(hnp_losses) > 0:
                            hnp_loss = (
                                common_loss_weight
                                * getattr(args, 'hnp_weight', 0.05)
                                * sum(hnp_losses) / len(hnp_losses)
                            )
                            if torch.isnan(hnp_loss).any():
                                nan_batch_counter += 1
                            else:
                                meter.update({
                                    'hnp_loss': hnp_loss.data,
                                    'hnp_margin': sum(item['margin'] for item in hnp_stats) / len(hnp_stats),
                                    'hnp_active': sum(item['active'] for item in hnp_stats) / len(hnp_stats),
                                    'hnp_reliable': sum(item['reliable'] for item in hnp_stats) / len(hnp_stats),
                                    'hnp_valid': sum(item['valid'] for item in hnp_stats),
                                })
                                total_loss += hnp_loss
