...
@torch.no_grad()
def _build_lrmd_basis(cma, common_dict, rank):
    if len(common_dict) < 2:
        return None, {}

    rgb_ids = torch.as_tensor(list(common_dict.keys()), dtype=torch.long, device=cma.vis_memory.device)
    ir_ids = torch.as_tensor(list(common_dict.values()), dtype=torch.long, device=cma.ir_memory.device)
    vis_prototypes = F.normalize(cma.vis_memory[rgb_ids], dim=1)
    ir_prototypes = F.normalize(cma.ir_memory[ir_ids], dim=1)
    modality_gap = vis_prototypes - ir_prototypes

    raw_pairs = modality_gap.shape[0]
    gap_norm = modality_gap.norm(dim=1)
    median = gap_norm.median()
    mad = (gap_norm - median).abs().median().clamp_min(1e-6)
    reliable_mask = gap_norm <= median + 2.5 * mad
    if reliable_mask.sum().item() >= max(2, int(rank)):
        modality_gap = modality_gap[reliable_mask]

    effective_rank = min(max(1, int(rank)), modality_gap.shape[0], modality_gap.shape[1])
    gram = modality_gap @ modality_gap.t()
    eigenvalues, eigenvectors = torch.linalg.eigh(gram)
    top_values = eigenvalues[-effective_rank:].clamp_min(1e-12)
    top_vectors = eigenvectors[:, -effective_rank:]
    basis = (top_vectors.t() @ modality_gap) / top_values.sqrt().unsqueeze(1)
    basis = F.normalize(basis, dim=1).detach()

    positive_energy = eigenvalues.clamp_min(0.0).sum().clamp_min(1e-12)
    stats = {
        'rank': effective_rank,
        'pairs': modality_gap.shape[0],
        'raw_pairs': raw_pairs,
        'energy': (top_values.sum() / positive_energy).item(),
    }
    return basis, stats

def _lrmd_alignment_loss(features, targets, basis):
    if features.numel() == 0 or targets.numel() == 0 or basis is None:
        return features.new_tensor(0.0)
    gap = F.normalize(features, dim=1) - F.normalize(targets, dim=1)
    projected_gap = gap @ basis.t()
    return projected_gap.pow(2).sum(dim=1).mean()
...
def train(args, model: Model, dataset, *arg):
    ...
    use_lrmd = (
        use_rgmfd
        and bool(getattr(args, 'enable_lrmd', 0))
        and not enable_phase1
        and epoch >= getattr(args, 'lrmd_start_epoch', 40)
    )
    lrmd_basis = None
    ...
    if 'wsl' in args.debug or not enable_phase1:
        ...
        if use_lrmd:
            update_interval = max(1, getattr(args, 'lrmd_update_interval', 5))
            last_update = getattr(cma, 'lrmd_basis_epoch', None)
            if last_update is None or epoch - last_update >= update_interval:
                basis, stats = _build_lrmd_basis(cma, common_dict, getattr(args, 'lrmd_rank', 8))
                if basis is not None:
                    cma.lrmd_basis = basis
                    cma.lrmd_basis_epoch = epoch
                    cma.lrmd_basis_stats = stats
            lrmd_basis = getattr(cma, 'lrmd_basis', None)
            lrmd_stats = getattr(cma, 'lrmd_basis_stats', {})
            if lrmd_basis is not None:
                logger(
                    'RG-LMD diag | rank:{} pairs:{}/{} energy:{:.2f}% basis_epoch:{}'.format(
                        lrmd_stats.get('rank', lrmd_basis.shape[0]),
                        lrmd_stats.get('pairs', len(common_dict)),
                        lrmd_stats.get('raw_pairs', len(common_dict)),
                        lrmd_stats.get('energy', 0.0) * 100,
                        getattr(cma, 'lrmd_basis_epoch', epoch),
                    )
                )
        ...
        remain_matched_rgb = list(remain_dict.keys())
            ...
            if enable_phase1:
                ...
                if args.debug == 'wsl':
                    ...
                    if (selected_tri_ir_ids.shape[0]!=0):
                        ...
                        if not torch.isnan(r2i_cmo_loss).any():
                    ...
                    if (selected_tri_rgb_ids.shape[0]!=0):
                        ...
                        if not torch.isnan(i2r_cmo_loss).any():
                    ...
                    if rgmfd_pack is not None and epoch >= getattr(args, 'rgmfd_start_epoch', 0):
                        ...
                        if selected_common_rgb_ids.shape[0] != 0:
                            ...
                            if use_lrmd and lrmd_basis is not None:
                                rel_losses.append(_lrmd_alignment_loss(selected_rgb_shared, selected_ir_memory, lrmd_basis))
                            else:
                        ...
                        if selected_common_ir_ids.shape[0] != 0:
                            ...
                            if use_lrmd and lrmd_basis is not None:
                                rel_losses.append(_lrmd_alignment_loss(selected_ir_shared, selected_rgb_memory, lrmd_basis))
                            else:
                                rel_losses.append(_cosine_alignment_loss(selected_ir_shared, selected_rgb_memory))
                        if len(rel_losses) > 0:
                            rel_weight = (
                                getattr(args, 'lrmd_weight', 0.02)
                                if use_lrmd and lrmd_basis is not None
                                else getattr(args, 'rgmfd_rel_weight', 0.0)
                            )
                            rgmfd_rel_loss = (
                                common_loss_weight
                                * rel_weight
                            ...
                            if not torch.isnan(rgmfd_rel_loss).any():
                                loss_name = 'lrmd_loss' if use_lrmd and lrmd_basis is not None else 'rgmfd_rel_loss'
                                meter.update({loss_name: rgmfd_rel_loss.data})
                ...
                if epoch >= remain_start_epoch:
                    ...
                    if remain_r2c_cls.shape[0] > 0:
                        ...
                        if not torch.isnan(weak_r2c_loss).any():
        ...
        if enable_phase1:
            ...
            if args.debug == 'wsl':
                ...
                if (selected_rgb_ids.shape[0]>0):
                    ...
                    if not torch.isnan(rgb_cross_loss).any():
