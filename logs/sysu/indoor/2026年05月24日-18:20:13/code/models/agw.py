...
class AGW(nn.Module):
    ...
    def _fuse_structured_feature(self, feature_map, global_feature):
        ...
        part_center = part_feature.mean(dim=1)
        part_dispersion = (part_feature - part_center.unsqueeze(1)).pow(2).mean(dim=(1, 2), keepdim=True)
        gate_input = torch.cat(
            [global_feature, structured_feature, part_dispersion.view(batch_size, 1)],
            dim=1,
        )
        gate = self.struct_gate(gate_input)
        fused_feature = global_feature + self.structured_weight * gate * structured_feature
