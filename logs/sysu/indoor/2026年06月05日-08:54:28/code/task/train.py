...
def _ddclae_loss(current_logits, peer_logits, temperature=2.0, mid=0.35, bandwidth=0.35, min_weight=0.05):
    if current_logits.numel() == 0 or peer_logits.numel() == 0:
        zero = current_logits.new_tensor(0.0)
        return zero, {'weight': zero, 'disagreement': zero, 'peer_teacher': zero}

    temperature = max(float(temperature), 1e-6)
    log_current = F.log_softmax(current_logits / temperature, dim=1)
    log_peer = F.log_softmax(peer_logits / temperature, dim=1)
    prob_current = log_current.exp()
    prob_peer = log_peer.exp()

    with torch.no_grad():
        max_entropy = max(float(np.log(current_logits.shape[1])), 1e-6)
        ent_current = -(prob_current * log_current).sum(dim=1) / max_entropy
        ent_peer = -(prob_peer * log_peer).sum(dim=1) / max_entropy
        peer_is_teacher = ent_peer < ent_current

        disagreement = 0.5 * (
            F.kl_div(log_current.detach(), prob_peer.detach(), reduction='none').sum(dim=1)
            + F.kl_div(log_peer.detach(), prob_current.detach(), reduction='none').sum(dim=1)
        )
        bandwidth = max(float(bandwidth), 1e-6)
        sample_weight = torch.exp(-torch.abs(disagreement - float(mid)) / bandwidth)
        sample_weight = sample_weight.clamp_min(float(min_weight))

    current_to_peer = F.kl_div(log_current, prob_peer.detach(), reduction='none').sum(dim=1)
    peer_to_current = F.kl_div(log_peer, prob_current.detach(), reduction='none').sum(dim=1)
    per_sample_loss = torch.where(peer_is_teacher, current_to_peer, peer_to_current)
    loss = (sample_weight * per_sample_loss).sum() / sample_weight.sum().clamp_min(1e-6)
    loss = loss * (temperature ** 2)

    stats = {
        'weight': sample_weight.mean().detach(),
        'disagreement': disagreement.mean().detach(),
        'peer_teacher': peer_is_teacher.float().mean().detach(),
    }
    return loss, stats
...
def train(args, model: Model, dataset, *arg):
    ...
    use_ddclae = bool(getattr(args, 'enable_ddclae', 0)) and epoch >= getattr(args, 'ddclae_start_epoch', 40)
            ...
            if enable_phase1:
                ...
                if args.debug == 'wsl':
                    ...
                    if use_ddclae:
                        ...
                        ddclae_stats = []
                        ddclae_kwargs = {
                            'temperature': getattr(args, 'ddclae_temperature', 2.0),
                            'mid': getattr(args, 'ddclae_mid', 0.35),
                            'bandwidth': getattr(args, 'ddclae_bandwidth', 0.35),
                            'min_weight': getattr(args, 'ddclae_min_weight', 0.05),
                        }
                        if selected_common_ir_ids.shape[0] != 0:
                            ir_ddclae_loss, ir_ddclae_stats = _ddclae_loss(
                                dtd_i2i_cls[common_ir_indices],
                                mem_r2i_cls,
                                **ddclae_kwargs,
                            )
                            ddclae_terms.append(ir_ddclae_loss)
                            ddclae_stats.append(ir_ddclae_stats)
                        if selected_common_rgb_ids.shape[0] != 0:
                            rgb_ddclae_loss, rgb_ddclae_stats = _ddclae_loss(
                                dtd_r2r_cls[common_rgb_indices],
                                mem_i2r_cls,
                                **ddclae_kwargs,
                            )
                            ddclae_terms.append(rgb_ddclae_loss)
                            ddclae_stats.append(rgb_ddclae_stats)
                        if len(ddclae_terms) > 0:
                            ddclae_loss = (
                                common_loss_weight
                                * getattr(args, 'ddclae_weight', 0.0)
                                * torch.stack(ddclae_terms).mean()
                            )
                            if torch.isnan(ddclae_loss).any():
                                nan_batch_counter += 1
                            else:
                                meter.update({
                                    'ddclae_loss': ddclae_loss.data,
                                    'ddclae_w': torch.stack([item['weight'] for item in ddclae_stats]).mean(),
                                    'ddclae_dis': torch.stack([item['disagreement'] for item in ddclae_stats]).mean(),
                                    'ddclae_peer_t': torch.stack([item['peer_teacher'] for item in ddclae_stats]).mean(),
                                })
                                total_loss += ddclae_loss
