...
class Model:
    def __init__(self, args):
        ...
        self.p2rnet = None
        if getattr(args, 'enable_p2r', 0):
            self.p2rnet = P2RNet(
                feature_dim=2048,
                hidden_dim=getattr(args, 'p2r_hidden_dim', 512),
                temperature=getattr(args, 'p2r_temp', 0.07),
                sinkhorn_iters=getattr(args, 'p2r_iters', 20),
                anchor_bias=getattr(args, 'p2r_anchor_bias', 4.0),
                candidate_bias=getattr(args, 'p2r_candidate_bias', 1.0),
                mask_penalty=getattr(args, 'p2r_mask_penalty', 8.0),
                dustbin_logit=getattr(args, 'p2r_dustbin_logit', 0.0),
            ).to(self.device)
        ...
        for key, value in self.classifier3.named_parameters():
            if value.requires_grad:
                params_phase2 += [{"params": [value], 
                            "lr": 2 * self.lr,
                            "weight_decay": self.weight_decay}]
        if self.p2rnet is not None:
            for key, value in self.p2rnet.named_parameters():
                if value.requires_grad:
                    params_phase2 += [{"params": [value],
                                "lr": self.lr,
                                "weight_decay": self.weight_decay}]
    ...
    def save_model(self, save_epoch, is_best):
        if is_best:
            ...
            if self.p2rnet is not None:
                all_state_dict['p2rnet'] = self.p2rnet.state_dict()
                ...
                if indexes:
                    ...
                    if self.resume or self.mode == 'test':
                        ...
                        try:
                            ...
                            pass
                        if self.p2rnet is not None and 'p2rnet' in loaded_dict:
                            self.p2rnet.load_state_dict(loaded_dict['p2rnet'], strict=False)
