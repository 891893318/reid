...
class CMA(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.enable_graymix_audit = bool(getattr(args, "enable_graymix_audit", 0))
        self.graymix_lambda = float(getattr(args, "graymix_lambda", 0.5))
        self.graymix_audit_start_epoch = int(getattr(args, "graymix_audit_start_epoch", 0))
        self.graymix_v2i = OrderedDict()
    ...
    def save(self,vis,ir,rgb_ids,ir_ids,rgb_idx,ir_idx,mode, rgb_features=None, ir_features=None,
             rgb_gt=None, ir_gt=None, graymix_cls=None):
    # vis: vis sample(v2i scores or vis features) ir: ir sample
        ...
        if self.mode != 'scores' and self.mode != 'features':
            ...
            if graymix_cls is not None:
                graymix_cls = torch.nn.functional.softmax(self.T*graymix_cls, dim=1)
        ###############################
        ...
        self.graymix = graymix_cls.detach().cpu().numpy() if graymix_cls is not None else None
    ...
    def get_label(self, epoch=None):
        if self.not_saved:# pass if 
            ...
            if self.mode == 'features':
                ...
                if self.graymix is not None:
                    self.graymix_v2i, _ = self._get_count_label(self.graymix, 'rgb')
                else:
                    self.graymix_v2i = OrderedDict()
                self.v2i = v2i_dict
                self.i2v = i2v_dict
    ...
    def audit_remain_relations(self, remain_dict, epoch):
        stats = {
            'enabled': False,
            'total': len(remain_dict),
            'kept': len(remain_dict),
            'keep_ratio': 1.0 if len(remain_dict) > 0 else 0.0,
            'graymix': {'valid': 0, 'correct': 0, 'acc': 0.0},
            'before': {'valid': 0, 'correct': 0, 'acc': 0.0},
            'after': {'valid': 0, 'correct': 0, 'acc': 0.0},
        }
        if (
            not self.enable_graymix_audit
            or epoch < self.graymix_audit_start_epoch
            or len(self.graymix_v2i) == 0
        ):
            return remain_dict, stats

        audited = OrderedDict(
            (r, i) for r, i in remain_dict.items()
            if self.graymix_v2i.get(int(r), None) == int(i)
        )
        stats['enabled'] = True
        stats['kept'] = len(audited)
        stats['keep_ratio'] = len(audited) / max(1, len(remain_dict))
        if hasattr(self, 'rgb_gt_by_label') and hasattr(self, 'ir_gt_by_label'):
            stats['graymix'] = self._pair_quality(self.graymix_v2i, 'r2i')
            stats['before'] = self._pair_quality(remain_dict, 'r2i')
            stats['after'] = self._pair_quality(audited, 'r2i')
    ...
    def extract(self, args, model, dataset, enable_graymix=False):
        '''
        ...
        with torch.no_grad():
            ...
            rgb_features, rgb_labels, rgb_gt, r2i_cls, rgb_idx, graymix_cls = self._extract_feature(
                model, rgb_loader, 'rgb', enable_graymix=enable_graymix
            )
            ir_features, ir_labels, ir_gt, i2r_cls, ir_idx, _ = self._extract_feature(
                model, ir_loader, 'ir'
            )

        # # //match by cls and save features to memory bank
        self.save(r2i_cls, i2r_cls, rgb_labels, ir_labels, rgb_idx,\
                 ir_idx, 'scores', rgb_features, ir_features, rgb_gt, ir_gt, graymix_cls)
        
    def _extract_feature(self, model, loader, modal, enable_graymix=False):

        print('extracting {} features'.format(modal))

        saved_features, saved_labels, saved_cls= None, None, None
        saved_gts, saved_idx= None, None
        saved_graymix_cls = None
        for imgs_list, infos in loader:
            ...
            if modal == 'rgb':
                ...
                graymix_cls = None
                if enable_graymix:
                    graymix_imgs = self._graymix(imgs)
                    _, graymix_bn_features = model.model(graymix_imgs)
                    graymix_cls, _ = model.classifier2(graymix_bn_features)
            elif modal == 'ir':
            ...
            if saved_features is None: 
                ...
                if graymix_cls is not None:
                    saved_graymix_cls = torch.cat((saved_graymix_cls, graymix_cls), dim=0)
        return saved_features, saved_labels, saved_gts, saved_cls, saved_idx, saved_graymix_cls

    def _graymix(self, normalized_imgs):
        mean = normalized_imgs.new_tensor([0.485, 0.456, 0.406]).view(1, 3, 1, 1)
        std = normalized_imgs.new_tensor([0.229, 0.224, 0.225]).view(1, 3, 1, 1)
        imgs = normalized_imgs * std + mean
        gray = imgs.mean(dim=1, keepdim=True)
        mixed = (1.0 - self.graymix_lambda) * imgs + self.graymix_lambda * gray
