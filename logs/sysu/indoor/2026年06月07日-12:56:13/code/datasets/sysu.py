...
class SYSU:
    ...
    def set_train_phase(self, enable_phase1):
        self.train_rgb.set_train_phase(enable_phase1)
...
class SYSU_train(data.Dataset):
    def __init__(self, args, data_path, modal = None):
        ...
        self.transform_color_phase1 = transform_color_ca
        self.transform_color_phase2 = transform_color_ca
        if self.modal == 'rgb' and getattr(args, 'enable_train_graymix', 0):
            graymix_transform = build_color_graymix(
                getattr(args, 'graymix_min_lambda', 0.3),
                getattr(args, 'graymix_lambda', 0.8),
            )
            self.transform_color_phase2 = RandomAuxiliaryTransform(
                transform_color_ca,
                graymix_transform,
                getattr(args, 'graymix_probability', 0.5),
            )
        self.transform_color_sa = self.transform_color_phase1
    ...
    def set_train_phase(self, enable_phase1):
        if self.modal == 'rgb' and not enable_phase1:
            self.transform_color_sa = self.transform_color_phase2
        else:
            self.transform_color_sa = self.transform_color_phase1
