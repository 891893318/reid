...
def save_checkpoint(args, model, epoch):
    path = os.path.join(args.save_path, "models", "stage1")
    makedir(path)
    ...
    torch.save(all_state_dict, os.path.join(path, 'model_{}.pth'.format(epoch)))
...
def infoEntropy(input):
    ...
    return -torch.mean(input * torch.log2(input.clamp_min(1e-12)))
