...
def _cross_modal_prototype_contrastive_loss(query_features, prototype_memory, positive_indices, args):
    ...
    temperature = max(getattr(args, 'cmpc_temperature', 0.1), 1e-6)
    min_positive_similarity = getattr(args, 'cmpc_min_positive_similarity', 0.2)
    prototype_norm = prototype_memory.detach().norm(dim=1)
    valid_prototypes = prototype_norm > 1e-12

    query_features = F.normalize(query_features, dim=1)
    prototypes = F.normalize(prototype_memory.detach(), dim=1)
    similarities = query_features @ prototypes.t()
    positive_similarity = similarities.gather(1, positive_indices[:, None]).squeeze(1)
    active = valid_prototypes[positive_indices] & (positive_similarity.detach() >= min_positive_similarity)

    logits = similarities / temperature
    logits = logits.masked_fill(~valid_prototypes[None, :], -1e4)
    per_sample_loss = F.cross_entropy(logits, positive_indices, reduction='none')
    loss = (per_sample_loss * active.float()).sum() / active.float().sum().clamp_min(1.0)

    negative_similarities = similarities.detach().masked_fill(~valid_prototypes[None, :], -1e4)
    ...
    stats = {
        ...
        'hard_negative_similarity': hard_negative_similarity.mean(),
        'margin': (positive_similarity.detach() - hard_negative_similarity).mean(),
    }
    return loss, stats
        ...
        if 'wsl' in args.debug:
            if enable_phase1:
                ...
                if args.debug == 'wsl':
                    ...
                    if epoch >= cmpc_start_epoch and cmpc_base_weight > 0:
                        ...
                        cmpc_stats = []
                        if selected_common_rgb_ids.shape[0] != 0:
                            rgb_cmpc_loss, rgb_cmpc_stats = _cross_modal_prototype_contrastive_loss(
                        ...
                        if selected_common_ir_ids.shape[0] != 0:
                            ir_cmpc_loss, ir_cmpc_stats = _cross_modal_prototype_contrastive_loss(
                        ...
                        if cmpc_stats:
                            ...
                            meter.update({
                                'cmpc_active_ratio': weighted_stat('active_ratio'),
