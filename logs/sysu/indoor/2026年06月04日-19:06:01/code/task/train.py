...
def _shared_proto_contrast_loss(
    ...
    pos_logits = logits[row_idx, target_labels]
    ...
    if prototypes.shape[0] > 1:
        ...
        max_neg_logits = neg_logits.max(dim=1).values
        if hard_k > 0 and hard_weight > 0:
            ...
            losses = losses + float(hard_weight) * F.softplus(
                hard_logits - pos_logits.unsqueeze(1)
            ).mean(dim=1)
    else:
        max_neg_logits = pos_logits.detach()

    with torch.no_grad():
        margin = pos_logits.detach() - max_neg_logits
        confidence = torch.sigmoid(margin)
        confidence_min = min(1.0, max(0.0, float(confidence_min)))
        confidence = confidence_min + (1.0 - confidence_min) * confidence
        sample_weights = confidence

    if weights is not None:
        relation_weights = weights.to(losses.device, dtype=losses.dtype).detach()[valid].clamp_min(0.0)
        sample_weights = sample_weights * relation_weights

    loss = (losses * sample_weights).sum() / sample_weights.sum().clamp_min(1e-6)
    stats = {
        'confidence': confidence.mean(),
        'margin': margin.mean(),
        'sample_weight': sample_weights.mean(),
    }
    return loss, stats

def _rgspc_weight_scale(args, epoch, start_epoch):
    decay_power = float(getattr(args, 'rgspc_decay_power', 4.0))
    if decay_power <= 0:
        return 1.0
    total_epochs = max(1, int(getattr(args, 'stage2_epoch', 120)) - int(start_epoch) - 1)
    progress = min(1.0, max(0.0, (float(epoch) - float(start_epoch)) / float(total_epochs)))
    return math.cos(0.5 * math.pi * progress) ** decay_power
...
def train(args, model: Model, dataset, *arg):
    ...
    rgspc_start_epoch = getattr(args, 'rgspc_start_epoch', 40)
    use_rgspc = (
        bool(getattr(args, 'enable_rgspc', 0))
        and use_rgmfd
        and not enable_phase1
        and epoch >= rgspc_start_epoch
    )
    use_rgrc = (
                ...
                if args.debug == 'wsl':
                    ...
                    if rgmfd_pack is not None and epoch >= getattr(args, 'rgmfd_start_epoch', 0):
                        ...
                        if use_rgspc:
                            ...
                            confidence_min = getattr(args, 'rgspc_confidence_min', 0.35)
                            rgspc_scale = _rgspc_weight_scale(args, epoch, rgspc_start_epoch)
                            if selected_common_rgb_ids.shape[0] != 0:
                                selected_rgb_shared = shared_bn[:2*bt][common_rgb_indices]
                                proto_loss, proto_stats = _shared_proto_contrast_loss(
                                    selected_rgb_shared,
                                ...
                                if proto_stats:
                                    proto_confidences.append(proto_stats['confidence'])
                                    proto_margins.append(proto_stats['margin'])
                                    proto_sample_weights.append(proto_stats['sample_weight'])
                            if selected_common_ir_ids.shape[0] != 0:
                                selected_ir_shared = shared_bn[2*bt:][common_ir_indices]
                                proto_loss, proto_stats = _shared_proto_contrast_loss(
                                    selected_ir_shared,
                            ...
                            if len(proto_losses) > 0:
                                ...
                                if torch.isnan(rgspc_loss).any():
                                    ...
                                    rgspc_stats = {
                                        'rgspc_loss': rgspc_loss.data,
                                        'rgspc_scale': rgspc_loss.new_tensor(rgspc_scale).data,
                                    }
                                    if len(proto_confidences) > 0:
                                        rgspc_stats['rgspc_conf'] = torch.stack(proto_confidences).mean().data
                                        rgspc_stats['rgspc_margin'] = torch.stack(proto_margins).mean().data
                                        rgspc_stats['rgspc_sample_w'] = torch.stack(proto_sample_weights).mean().data
                                    meter.update(rgspc_stats)
