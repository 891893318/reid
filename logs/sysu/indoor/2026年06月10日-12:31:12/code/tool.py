...
def infer_method(args: dict[str, Any]) -> str:
    if value_is_enabled(args.get("enable_rgmfd")):
        if value_is_positive(args.get("uclae_weight")):
            return "RG-MFD+UCLAE"
...
def build_short_params(args: dict[str, Any]) -> str:
    pairs = [
        ...
        ("uclae_weight", "uclae_w", None),
        ("uclae_temperature", "uclae_t", None),
        ("uclae_conflict_temperature", "uclae_conf_t", None),
        ("uclae_min_teacher_confidence", "uclae_min", None),
        ("uclae_require_native_correct", "uclae_correct", None),
        ("enable_rdl", "rdl", "-"),
        ("enable_trrm", "trrm", "-"),
