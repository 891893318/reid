...
def _soft_relation_distill_loss(logits, target_logits, labels, temperature, min_target_prob):
    if logits.numel() == 0 or target_logits.numel() == 0:
        return None, {}

    with torch.no_grad():
        target_label_prob = F.softmax(target_logits, dim=1).gather(
            1, labels.unsqueeze(1)
        ).squeeze(1)
        confident = target_label_prob >= float(min_target_prob)
    if not confident.any():
        return None, {}

    logits = logits[confident]
    target_logits = target_logits[confident]
    labels = labels[confident]
    target_label_prob = target_label_prob[confident]
    ...
    weights = target_label_prob / target_label_prob.max().clamp_min(1e-12)
    loss = (weights * per_sample).sum() / weights.sum().clamp_min(1e-12)

    with torch.no_grad():
        pred_label_prob = F.softmax(logits, dim=1).gather(1, labels.unsqueeze(1)).mean()
        anchors = logits.new_tensor(float(logits.shape[0]))
    return loss, {
        'anchors': anchors,
        'target_prob': target_label_prob.mean(),
        'pred_prob': pred_label_prob,
    }


def _common_prototype_distill_loss(
    model,
    rgb_logits,
    ir_logits,
    rgb_ids,
    ...
    rgb_valid = rgb_targets >= 0
    if rgb_valid.any():
        ir_memory_valid = ir_memory.norm(dim=1) > 1e-12
        rgb_valid = rgb_valid & ir_memory_valid[rgb_targets.clamp(min=0)]
    if rgb_valid.any():
        target_logits, _ = model.classifier1(ir_memory[rgb_targets[rgb_valid]].detach())
        rgb_loss, rgb_stats = _soft_relation_distill_loss(
            rgb_logits[rgb_valid],
            target_logits,
            rgb_ids[rgb_valid],
            temperature,
            min_target_prob,
    ...
    ir_valid = ir_targets >= 0
    if ir_valid.any():
        vis_memory_valid = vis_memory.norm(dim=1) > 1e-12
        ir_valid = ir_valid & vis_memory_valid[ir_targets.clamp(min=0)]
    if ir_valid.any():
        target_logits, _ = model.classifier2(vis_memory[ir_targets[ir_valid]].detach())
        ir_loss, ir_stats = _soft_relation_distill_loss(
            ir_logits[ir_valid],
            target_logits,
            ir_ids[ir_valid],
            temperature,
    ...
    loss = sum(losses) / len(losses)
    total_anchors = sum(item['anchors'] for item in stats)
    target_prob = sum(item['target_prob'] * item['anchors'] for item in stats)
    pred_prob = sum(item['pred_prob'] * item['anchors'] for item in stats)
    target_prob = target_prob / total_anchors.clamp_min(1e-12)
    pred_prob = pred_prob / total_anchors.clamp_min(1e-12)
    return loss, {
...
def train(args, model: Model, dataset, *arg):
    ...
    for (rgb_imgs, ca_imgs, color_info), (ir_imgs, aug_imgs, ir_info) in zip(rgb_loader, ir_loader):
        ...
        if args.debug == 'wsl' and enable_phase1:
            ...
            cmo_weight = getattr(args, 'uprt_cmo_weight', 0.0)
            cmo_scale = _warmup_scale(
                epoch,
                getattr(args, 'uprt_cmo_start_epoch', 50),
                getattr(args, 'uprt_cmo_warmup_epochs', 10),
            )
            if getattr(args, 'enable_uprt', 0) and cmo_weight > 0 and cmo_scale > 0:
                cmo_loss, cmo_stats = _common_prototype_distill_loss(
                    model,
                    r2r_cls,
                    i2i_cls,
                    rgb_ids,
                    ...
                    cma.uprt_vis_memory,
                    cma.uprt_ir_memory,
                    getattr(args, 'uprt_cmo_temperature', 2.0),
                    getattr(args, 'uprt_cmo_min_target_prob', 0.05),
                )
                if cmo_loss is not None:
                    cmo_loss = cmo_weight * cmo_scale * cmo_loss
                    total_loss = total_loss + cmo_loss
                    meter.update({
                        'uprt_cmo_loss': cmo_loss.data,
                        'uprt_cmo_anchors': cmo_stats['anchors'],
                        'uprt_cmo_target_prob': cmo_stats['target_prob'],
                        'uprt_cmo_pred_prob': cmo_stats['pred_prob'],
                    })
