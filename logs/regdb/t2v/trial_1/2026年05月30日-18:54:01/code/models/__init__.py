...
from .relation import UOTCrossModalRelationTransformer
_models = {
...
class Model:
    def __init__(self, args):
        ...
        if getattr(args, "enable_uotcrt", 0):
            self.relation_module = UOTCrossModalRelationTransformer(
                feature_dim=2048,
                hidden_dim=getattr(args, "uotcrt_hidden_dim", 512),
                temperature=getattr(args, "uotcrt_temp", 0.07),
                sinkhorn_iters=getattr(args, "uotcrt_iters", 30),
                residual_scale=getattr(args, "uotcrt_residual_scale", 0.2),
            ).to(self.device)
    ...
    def _init_optimizer(self):
        ...
        if self.relation_module is not None:
            for key, value in self.relation_module.named_parameters():
                if value.requires_grad:
                    params_phase2 += [{"params": [value],
                                "lr": self.lr,
                                "weight_decay": self.weight_decay}]
    ...
    def save_model(self, save_epoch, is_best):
        if is_best:
            ...
            if self.relation_module is not None:
                all_state_dict['relation_module'] = self.relation_module.state_dict()
                ...
                if indexes:
                    ...
                    if self.resume or self.mode == 'test':
                        ...
                        try:
                            ...
                            pass
                        if self.relation_module is not None and 'relation_module' in loaded_dict:
                            self.relation_module.load_state_dict(loaded_dict['relation_module'], strict=False)
