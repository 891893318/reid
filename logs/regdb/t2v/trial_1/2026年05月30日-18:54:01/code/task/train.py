...
def _soft_cross_entropy(logits, soft_targets, confidence=None):
    ...
    confidence = confidence.detach().clamp_min(0.0)
    return (per_sample * confidence).sum() / confidence.sum().clamp_min(1e-12)

def _uotcrt_cycle_loss(transport):
    if transport.numel() == 0:
        return transport.new_tensor(0.0)
    rgb_cycle = torch.matmul(transport, transport.t())
    ir_cycle = torch.matmul(transport.t(), transport)
    labels = torch.arange(transport.shape[0], device=transport.device)
    return 0.5 * (
        F.cross_entropy(rgb_cycle, labels)
        + F.cross_entropy(ir_cycle, labels)
    )

def _uotcrt_diag(pack, common_dict, num_classes):
    transport = pack['transport']
    eps = 1e-12
    entropy = -(transport * transport.clamp_min(eps).log()).sum(dim=1)
    entropy = entropy.mean() / np.log(max(num_classes, 2))
    common_prob = transport.new_tensor(0.0)
    if len(common_dict) > 0:
        rows = torch.as_tensor(list(common_dict.keys()), dtype=torch.long, device=transport.device)
        cols = torch.as_tensor(list(common_dict.values()), dtype=torch.long, device=transport.device)
        common_prob = transport[rows, cols].mean()
    return {
        'row_conf': pack['row_conf'].mean(),
        'col_conf': pack['col_conf'].mean(),
        'entropy': entropy,
...
def train(args, model: Model, dataset, *arg):
    ...
    if 'wsl' in args.debug or not enable_phase1:
        ...
        use_uotcrt = (
            (not enable_phase1)
            and args.debug == 'wsl'
            and bool(getattr(args, 'enable_uotcrt', 0))
            and epoch >= getattr(args, 'uotcrt_start_epoch', 0)
            and getattr(model, 'relation_module', None) is not None
        )
        if use_uotcrt:
            with torch.no_grad():
                uotcrt_pack = model.relation_module(
                    cma.vis_memory.to(model.device),
                    cma.ir_memory.to(model.device),
                )
                uotcrt_stats = _uotcrt_diag(uotcrt_pack, common_dict, args.num_classes)
            logger(
                'UOT-CRT diag | row_conf:{:.4f} col_conf:{:.4f} entropy:{:.4f} common_p:{:.4f}'.format(
                    uotcrt_stats['row_conf'].item(),
                    uotcrt_stats['col_conf'].item(),
                    uotcrt_stats['entropy'].item(),
                    uotcrt_stats['common_prob'].item(),
                )
            )
    # ======================================================
    # 2. 网络前向传播与模型损失计算主循环
            ...
            if enable_phase1:
                ...
                if args.debug == 'wsl':
                    ...
                    if use_uotcrt:
                        ...
                        transport = uotcrt_pack['transport']
                        rgb_soft_targets = transport[rgb_ids]
                        ir_soft_targets = transport.t()[ir_ids]
                        conf_power = getattr(args, 'uotcrt_conf_power', 1.0)
                        rgb_conf = uotcrt_pack['row_conf'][rgb_ids].pow(conf_power)
                        ir_conf = uotcrt_pack['col_conf'][ir_ids].pow(conf_power)

                        uotcrt_cm_loss = getattr(args, 'uotcrt_cm_weight', 0.0) * (
                            _soft_cross_entropy(r2c_cls, rgb_soft_targets.detach(), rgb_conf)
                            + _soft_cross_entropy(i2c_cls, ir_soft_targets.detach(), ir_conf)
                        )
                        relation_features = rgmfd_pack['shared_bn'] if rgmfd_pack is not None else bn_features
                        rgb_relation_features = relation_features[:2*bt]
                        ir_relation_features = relation_features[2*bt:]
                        rgb_target_proto = torch.matmul(rgb_soft_targets, uotcrt_pack['ir_refined'])
                        ir_target_proto = torch.matmul(ir_soft_targets, uotcrt_pack['rgb_refined'])
                        uotcrt_proto_loss = getattr(args, 'uotcrt_proto_weight', 0.0) * (
                            _cosine_alignment_loss(rgb_relation_features, rgb_target_proto)
                            + _cosine_alignment_loss(ir_relation_features, ir_target_proto)
                        )
                        uotcrt_cycle_loss = (
                            getattr(args, 'uotcrt_cycle_weight', 0.0)
                            * _uotcrt_cycle_loss(transport)
                        )
                        uotcrt_loss = uotcrt_cm_loss + uotcrt_proto_loss + uotcrt_cycle_loss
                        if torch.isnan(uotcrt_loss).any():
                            nan_batch_counter += 1
                        else:
                            meter.update({
                                'uotcrt_cm_loss': uotcrt_cm_loss.data,
                                'uotcrt_proto_loss': uotcrt_proto_loss.data,
                                'uotcrt_cycle_loss': uotcrt_cycle_loss.data,
                            })
                            total_loss += uotcrt_loss
