...
class CMA(nn.Module):
    ...
    def _get_hybrid_label(self):
        ...
        row_pairs, _ = self._get_label(self.vis_scores.numpy(), 'rgb')
        col_pairs, _ = self._get_label(self.ir_scores.numpy(), 'ir')
        ...
        for r, i in row_pairs.items():
            if i in col_pairs.keys() and col_pairs[i] == r:
                common_dict[r] = i
            elif r not in col_pairs.values() and i not in col_pairs.keys():
                specific_dict[r] = i
            else:
                remain_dict[r] = i
        for i, r in col_pairs.items():
            if (r, i) in common_dict.items():
                continue
            elif r not in row_pairs.values() and i not in row_pairs.keys():
                specific_dict[r] = i
            else:
                remain_dict[r] = i

        row_assign = torch.arange(self.num_classes, device=self.device)
        col_assign = torch.arange(self.num_classes, device=self.device)
        for r, i in row_pairs.items():
            row_assign[r] = i
        for i, r in col_pairs.items():
            col_assign[i] = r
