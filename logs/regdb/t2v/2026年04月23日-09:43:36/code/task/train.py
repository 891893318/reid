    ...
    for (rgb_imgs, ca_imgs, color_info), (ir_imgs, aug_imgs,ir_info) in zip(rgb_loader, ir_loader):
        ...
        if 'wsl' in args.debug:
            if enable_phase1:
                ...
                # 直接在测试会用到的专家头上做 BCPT 对齐，避免只训练 classifier3
                # ----------------
                dtd_features = bn_features.detach()

                # 计算跨模态的基础 ID 误差，保持 phase2 继续更新 backbone
                r2r_id_loss = model.pid_criterion(r2r_cls, rgb_ids)
                i2i_id_loss = model.pid_criterion(i2i_cls, ir_ids)
                meter.update({'r2r_id_loss':r2r_id_loss.data,
                            'i2i_id_loss':i2i_id_loss.data})
                ...
                rgb_transport_targets = sparse_targets(transport_rgb[rgb_ids], cma.topk)
                ir_transport_targets = sparse_targets(transport_ir[ir_ids], cma.topk)
                ...
                if args.debug == 'wsl':
                    # ================ A. BCPT 软传输监督 ================
                    rgb_bcpt_loss = weighted_soft_ce(r2i_cls, rgb_transport_targets, rgb_transport_weights)
                    ir_bcpt_loss = weighted_soft_ce(i2r_cls, ir_transport_targets, ir_transport_weights)
            ...
            if args.debug == 'wsl':
                ...
                if selected_rgb_ids.shape[0] > 0:
                    rgb_cross_loss = weighted_soft_ce(
                        selected_r2i_cls,
                        sparse_targets(specific_rm[selected_rgb_ids], cma.topk),
                ...
                if selected_ir_ids.shape[0] > 0:
                    ir_cross_loss = weighted_soft_ce(
                        selected_i2r_cls,
                        sparse_targets(transport_ir[selected_ir_ids], cma.topk),
