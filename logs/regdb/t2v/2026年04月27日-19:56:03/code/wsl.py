...
class CMA(nn.Module):
    ...
    @torch.no_grad()
    def get_soft_relation(self, args):
        if self.not_saved:
            raise RuntimeError("CMA memory is empty. Call extract() before get_soft_relation().")

        vis_scores = torch.from_numpy(self.vis).to(self.device).float()
        ir_scores = torch.from_numpy(self.ir).to(self.device).float()
        rgb_ids = self.rgb_ids.to(self.device).long()
        ir_ids = self.ir_ids.to(self.device).long()

        label_v2r = self._aggregate_scores_by_label(vis_scores, rgb_ids)
        label_r2v = self._aggregate_scores_by_label(ir_scores, ir_ids)

        vis_proto = F.normalize(self.vis_memory, dim=1)
        ir_proto = F.normalize(self.ir_memory, dim=1)
        proto_sim = torch.matmul(vis_proto, ir_proto.t())

        relation_score = (
            args.soft_lambda_v2r * label_v2r
            + args.soft_lambda_r2v * label_r2v.t()
            + args.soft_lambda_proto * proto_sim
        )

        valid_rgb = torch.bincount(rgb_ids, minlength=self.num_classes) > 0
        valid_ir = torch.bincount(ir_ids, minlength=self.num_classes) > 0
        relation_score = relation_score.masked_fill(~valid_ir.unsqueeze(0), -1e4)

        q_v2r = torch.softmax(relation_score / args.soft_relation_temp, dim=1)
        q_v2r = q_v2r * valid_ir.unsqueeze(0).float()
        q_v2r = q_v2r / q_v2r.sum(dim=1, keepdim=True).clamp_min(1e-12)
        w_v2r = self._entropy_confidence(q_v2r, valid_ir.sum())
        w_v2r = w_v2r * valid_rgb.float()

        relation_score_t = relation_score.t().masked_fill(~valid_rgb.unsqueeze(0), -1e4)
        q_r2v = torch.softmax(relation_score_t / args.soft_relation_temp, dim=1)
        q_r2v = q_r2v * valid_rgb.unsqueeze(0).float()
        q_r2v = q_r2v / q_r2v.sum(dim=1, keepdim=True).clamp_min(1e-12)
        w_r2v = self._entropy_confidence(q_r2v, valid_rgb.sum())
        w_r2v = w_r2v * valid_ir.float()
    ...
    def _aggregate_scores_by_label(self, scores, labels):
        aggregated = torch.zeros(self.num_classes, scores.size(1), device=scores.device, dtype=scores.dtype)
        counts = torch.bincount(labels, minlength=self.num_classes).float().unsqueeze(1).to(scores.device)
        aggregated.index_add_(0, labels, scores)
        aggregated = aggregated / counts.clamp_min(1.0)
        return aggregated

    def _entropy_confidence(self, probs, class_count):
        if int(class_count) <= 1:
            return torch.ones(probs.size(0), device=probs.device, dtype=probs.dtype)
        entropy = -(probs * probs.clamp_min(1e-12).log()).sum(dim=1)
        return 1.0 - entropy / np.log(float(class_count))
