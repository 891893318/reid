...
class CMA(nn.Module):
    ...
    def _rgrc_score(self, args, relation_type, rgb_label, ir_label, previous_pairs, previous_scores):
        ...
        pair_key = (int(rgb_label), int(ir_label))
        score_key = (relation_type, pair_key[0], pair_key[1])
        consistency_score = self._relation_consistency_score(relation_type, pair_key, previous_pairs)
        ...
        if score_key in previous_scores:
            raw_score = momentum * previous_scores[score_key] + (1.0 - momentum) * raw_score
        return max(0.0, min(1.0, raw_score))

    def _rgrc_weight(self, score, min_weight):
        return max(0.0, min(1.0, min_weight + (1.0 - min_weight) * score))

    def _rgrc_bucket_stats(self, scores, weights, kept, total, suppressed=0):
        if total == 0:
            return {
                'num': 0,
                'kept': 0,
    ...
    def relation_correction(self, args, common_dict, specific_dict, remain_dict, device):
        ...
        next_scores = {}
        stats = {}
        common_tail_ratio = getattr(args, 'rgrc_common_tail_ratio', 0.15)
        common_tail_ratio = max(0.0, min(1.0, float(common_tail_ratio)))
        ...
        for relation_type, pair_dict in relation_dicts.items():
            ...
            for rgb_label, ir_label in pair_dict.items():
                ...
                scores.append(score)
                next_scores[(relation_type, key[0], key[1])] = score
                if score < thresholds[relation_type]:
                    continue
                kept_items.append((key, score))

            suppressed = 0
            suppressed_common = set()
            if relation_type == 'common' and len(kept_items) > 0 and common_tail_ratio > 0:
                tail_count = int(np.ceil(len(kept_items) * common_tail_ratio))
                tail_count = max(1, min(tail_count, len(kept_items)))
                sorted_items = sorted(kept_items, key=lambda item: item[1])
                suppressed_common = set(key for key, _ in sorted_items[:tail_count])

            for key, score in kept_items:
                if relation_type == 'common':
                    if key in suppressed_common:
            ...
            stats[relation_type] = self._rgrc_bucket_stats(
                scores, weights, len(kept_items), len(pair_dict), suppressed
            )
