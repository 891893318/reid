...
def rampup_weight(step, start, warmup):
    if step < start:
        return 0.0
    if warmup <= 0:
        return 1.0
    return min(1.0, float(step - start + 1) / float(warmup))
    ...
    for (rgb_imgs, ca_imgs, color_info), (ir_imgs, aug_imgs,ir_info) in zip(rgb_loader, ir_loader):
        ...
        if 'wsl' in args.debug:
            if enable_phase1:
                ...
                # ----------------
                # 第二阶段 Phase 2: 恢复强基线路由，并在监督接入节奏上做轻量渐进门控
                ...
                phase2_epoch = epoch - args.stage1_epoch
                specific_gate = rampup_weight(phase2_epoch, 0, args.specific_warmup)
                remain_gate = rampup_weight(phase2_epoch, args.remain_start, args.remain_warmup)
                ###############################################################
                if args.debug == 'wsl':
                    # ================ A. common 的跨模态三元组 ================
                    ...
                    translated_cmo_rgb_label = torch.nonzero(common_rm[selected_common_rgb_ids])[:,-1] if selected_common_rgb_ids.numel() else selected_common_rgb_ids
                    translated_cmo_ir_label = torch.nonzero(common_rm.t()[selected_common_ir_ids])[:,-1] if selected_common_ir_ids.numel() else selected_common_ir_ids
                    ...
                    if selected_common_ir_ids.shape[0] != 0:
                        ...
                        r2i_cmo_loss = w_r2i * torch.nn.functional.mse_loss(dtd_i2i_cls[common_ir_indices], mem_r2i_cls)
                    ...
                    if selected_common_rgb_ids.shape[0] != 0:
                        ...
                        i2r_cmo_loss = w_i2r * torch.nn.functional.mse_loss(dtd_r2r_cls[common_rgb_indices], mem_i2r_cls)
                ...
                if remain_gate > 0:
                    ...
                    if remain_rgb_ids.shape[0] > 0:
                        weak_r2c_loss = args.weak_weight * remain_gate * model.weak_criterion(remain_r2c_cls, remain_rm[remain_rgb_ids])
                    ...
                    if remain_ir_ids.shape[0] > 0:
                        weak_i2c_loss = args.weak_weight * remain_gate * model.weak_criterion(remain_i2c_cls, remain_col_rm[remain_ir_ids])
        ...
        if enable_phase1:
            ...
            if args.debug == 'wsl':
                # ================ D. specific 的渐进门控监督 ================
                specific_rgb_only = specific_rgb_indices ^ common_rgb_indices
                specific_ir_only = specific_ir_indices ^ common_ir_indices
                selected_ir_ids = ir_ids[specific_ir_only]
                selected_rgb_ids = rgb_ids[specific_rgb_only]
                selected_i2c_cls = i2c_cls[specific_ir_only]
                selected_r2c_cls = r2c_cls[specific_rgb_only]

                if selected_rgb_ids.shape[0] > 0 and specific_gate > 0:
                    rgb_cross_loss = specific_gate * model.pid_criterion(selected_r2c_cls, specific_rm[selected_rgb_ids])
                    if torch.isnan(rgb_cross_loss).any():
                        nan_batch_counter+=1
                    else:
                        meter.update({'rgb_cross_loss':rgb_cross_loss.data})
                        total_loss += rgb_cross_loss
                ir_cross_loss = specific_gate * model.pid_criterion(i2c_cls, ir_ids)
                if selected_ir_ids.shape[0] > 0 or specific_gate > 0:
