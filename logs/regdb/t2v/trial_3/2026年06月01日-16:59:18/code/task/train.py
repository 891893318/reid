...
def _prediction_certainty(logits):
    probs = F.softmax(logits, dim=1)
    entropy = -(probs * probs.clamp_min(1e-12).log()).sum(dim=1)
    max_entropy = np.log(max(2, probs.shape[1]))
    return (1.0 - entropy / max_entropy).clamp(0.0, 1.0)

def _prediction_margin(logits):
    probs = F.softmax(logits, dim=1)
    top_values = probs.topk(k=min(2, probs.shape[1]), dim=1).values
    if top_values.shape[1] == 1:
        return top_values[:, 0].clamp(0.0, 1.0)
    return (top_values[:, 0] - top_values[:, 1]).clamp(0.0, 1.0)

def _weighted_bidirectional_kl(logits_a, logits_b, weights, temperature=3.0):
    if logits_a.shape[0] == 0 or logits_b.shape[0] == 0:
    ...
    prob_a = log_a.exp()
    prob_b = log_b.exp()
    kl_ab = F.kl_div(log_a, prob_b.detach(), reduction='none').sum(dim=1)
    kl_ba = F.kl_div(log_b, prob_a.detach(), reduction='none').sum(dim=1)
    loss = 0.5 * (kl_ab + kl_ba) * (temperature ** 2)
    weights = weights.to(loss.device).float().detach()
    return (loss * weights).sum() / weights.sum().clamp_min(1e-6)

def _weighted_oneway_kl(logits, target_logits, weights, temperature=3.0):
    if logits.shape[0] == 0 or target_logits.shape[0] == 0:
        return logits.new_tensor(0.0)
    temperature = max(float(temperature), 1e-6)
    log_probs = F.log_softmax(logits / temperature, dim=1)
    target_probs = F.softmax(target_logits.detach() / temperature, dim=1)
    loss = F.kl_div(log_probs, target_probs, reduction='none').sum(dim=1) * (temperature ** 2)
    weights = weights.to(loss.device).float().detach()
    return (loss * weights).sum() / weights.sum().clamp_min(1e-6)

def _weighted_cosine_alignment_loss(features, targets, weights):
    if features.numel() == 0 or targets.numel() == 0:
        return features.new_tensor(0.0)
    loss = 1.0 - F.cosine_similarity(features, targets.detach(), dim=1)
    weights = weights.to(loss.device).float().detach()
    return (loss * weights).sum() / weights.sum().clamp_min(1e-6)

def _crccl_relation_weights(args, current_logits, memory_logits, current_features, memory_features,
                            rgb_pair_ids, ir_pair_ids, previous_common_pairs):
    if current_logits.shape[0] == 0:
        return current_logits.new_tensor([])
    with torch.no_grad():
        proto_score = (F.cosine_similarity(
            F.normalize(current_features.float(), dim=1),
            F.normalize(memory_features.float(), dim=1),
            dim=1,
        ) + 1.0) * 0.5
        margin = 0.5 * (
            _prediction_margin(current_logits.detach())
            + _prediction_margin(memory_logits.detach())
        )
        certainty = 0.5 * (
            _prediction_certainty(current_logits.detach())
            + _prediction_certainty(memory_logits.detach())
        )
        if rgb_pair_ids.numel() == 0:
            stability = torch.zeros_like(proto_score)
        else:
            stable_values = [
                1.0 if (int(r), int(i)) in previous_common_pairs else 0.0
                for r, i in zip(rgb_pair_ids.detach().cpu().tolist(), ir_pair_ids.detach().cpu().tolist())
            ]
            stability = torch.as_tensor(stable_values, dtype=proto_score.dtype, device=proto_score.device)

        proto_w = getattr(args, 'crccl_proto_weight', 0.4)
        margin_w = getattr(args, 'crccl_margin_weight', 0.3)
        certainty_w = getattr(args, 'crccl_certainty_weight', 0.2)
        stability_w = getattr(args, 'crccl_stability_weight', 0.1)
        norm = max(1e-6, proto_w + margin_w + certainty_w + stability_w)
        score = (
            proto_w * proto_score
            + margin_w * margin
            + certainty_w * certainty
            + stability_w * stability
        ) / norm
        min_weight = getattr(args, 'crccl_min_weight', 0.2)
        weights = min_weight + (1.0 - min_weight) * score.clamp(0.0, 1.0)
    return weights.detach()

def _crccl_counterfactual_loss(args, model, shared_features, specific_features, reference_logits, weights):
    if shared_features.shape[0] <= 1:
        return shared_features.new_tensor(0.0)
    shuffled = specific_features[torch.randperm(specific_features.shape[0], device=specific_features.device)]
    scale = getattr(args, 'crccl_cf_scale', 0.1)
    cf_logits = model.classifier3(shared_features + scale * shuffled.detach())[0]
    return _weighted_oneway_kl(
        cf_logits,
        reference_logits,
        weights,
        temperature=getattr(args, 'crccl_temp', 3.0),
    )

def _as_pair_set(pair_dict):
    return set((int(k), int(v)) for k, v in pair_dict.items())

def _add_pairs_to_mask(mask, pair_dict):
    num_rows, num_cols = mask.shape
    for row, col in pair_dict.items():
        row, col = int(row), int(col)
        if 0 <= row < num_rows and 0 <= col < num_cols:
            mask[row, col] = True

def _build_p2r_masks(args, model, cma, epoch, common_dict, specific_dict, remain_dict,
                     r2i_pair_dict, i2r_pair_dict):
    device = model.device
    num_classes = args.num_classes
    anchor_mask = torch.zeros((num_classes, num_classes), dtype=torch.bool, device=device)
    candidate_mask = torch.zeros_like(anchor_mask)

    _add_pairs_to_mask(candidate_mask, r2i_pair_dict)
    _add_pairs_to_mask(candidate_mask, {r: i for i, r in i2r_pair_dict.items()})
    _add_pairs_to_mask(candidate_mask, common_dict)
    _add_pairs_to_mask(candidate_mask, specific_dict)
    _add_pairs_to_mask(candidate_mask, remain_dict)

    with torch.no_grad():
        vis_memory = F.normalize(cma.vis_memory.to(device).float(), dim=1)
        ir_memory = F.normalize(cma.ir_memory.to(device).float(), dim=1)
        proto_score = (torch.matmul(vis_memory, ir_memory.t()) + 1.0) * 0.5
        topk = min(max(1, getattr(args, 'p2r_candidate_topk', 5)), num_classes)
        rgb_topk = proto_score.topk(k=topk, dim=1).indices
        ir_topk = proto_score.topk(k=topk, dim=0).indices
        row_ids = torch.arange(num_classes, device=device).unsqueeze(1).expand_as(rgb_topk)
        candidate_mask[row_ids.reshape(-1), rgb_topk.reshape(-1)] = True
        col_ids = torch.arange(num_classes, device=device).unsqueeze(0).expand_as(ir_topk)
        candidate_mask[ir_topk.reshape(-1), col_ids.reshape(-1)] = True

        prev_common = getattr(cma, 'p2r_prev_common_pairs', set())
        current_common = _as_pair_set(common_dict)
        threshold = getattr(args, 'p2r_anchor_threshold', 0.85)
        anchor_items = []
        for row, col in current_common:
            stable = 1.0 if (row, col) in prev_common else 0.0
            score = 0.7 * float(proto_score[row, col].item()) + 0.3 * stable
            anchor_items.append((score, row, col))

        selected_anchors = [item for item in anchor_items if item[0] >= threshold]
        min_anchors = max(0, getattr(args, 'p2r_min_anchors', 32))
        if len(selected_anchors) < min(min_anchors, len(anchor_items)):
            anchor_items = sorted(anchor_items, key=lambda x: x[0], reverse=True)
            selected_anchors = anchor_items[:min(min_anchors, len(anchor_items))]
        for _, row, col in selected_anchors:
            anchor_mask[row, col] = True
            candidate_mask[row, col] = True

        cma.p2r_prev_common_pairs = current_common
    ...
    stats = {
        ...
        'common_pairs': len(common_dict),
    }
    return anchor_mask, candidate_mask, stats

def _p2r_anchor_loss(p2r_pack, anchor_mask):
    relation = p2r_pack['relation']
    if anchor_mask is None or not anchor_mask.any():
        return relation.new_tensor(0.0)
    return -relation[anchor_mask].clamp_min(1e-8).log().mean()

def _p2r_uncertainty_loss(p2r_pack, anchor_mask):
    uncertainty = p2r_pack['uncertainty']
    if anchor_mask is None or not anchor_mask.any():
        return uncertainty.new_tensor(0.0)
    return uncertainty[anchor_mask].mean()

def _p2r_soft_relation_ce(logits, relation, ids, direction='r2i', topk=5):
    if logits.shape[0] == 0 or ids.shape[0] == 0:
        return logits.new_tensor(0.0), 0, logits.new_tensor(0.0)

    if direction == 'r2i':
        soft_targets = relation[ids]
    elif direction == 'i2r':
        soft_targets = relation.t()[ids]
    else:
        raise ValueError('invalid P2R relation direction')

    if soft_targets.shape[1] == 0:
        return logits.new_tensor(0.0), 0, logits.new_tensor(0.0)

    topk = min(max(1, topk), soft_targets.shape[1])
    if topk < soft_targets.shape[1]:
        top_values, top_indices = soft_targets.topk(k=topk, dim=1)
        sparse_targets = torch.zeros_like(soft_targets)
        sparse_targets.scatter_(1, top_indices, top_values)
        soft_targets = sparse_targets

    mass = soft_targets.sum(dim=1, keepdim=True)
    valid = mass.squeeze(1) > 1e-8
    if valid.sum().item() == 0:
        return logits.new_tensor(0.0), 0, logits.new_tensor(0.0)

    soft_targets = (soft_targets[valid] / mass[valid]).detach()
    log_probs = F.log_softmax(logits[valid], dim=1)
    loss = -(soft_targets * log_probs).sum(dim=1).mean()
    confidence = soft_targets.max(dim=1).values.mean()
    return loss, int(valid.sum().item()), confidence.detach()

def _p2r_refine_loss(p2r_pack, shared_features, rgb_ids, ir_ids, bt):
    rgb_refined = p2r_pack['rgb_refined']
    ir_refined = p2r_pack['ir_refined']
    rgb_target = rgb_refined[rgb_ids].detach()
    ir_target = ir_refined[ir_ids].detach()
    rgb_loss = _cosine_alignment_loss(shared_features[:2 * bt], rgb_target)
    ir_loss = _cosine_alignment_loss(shared_features[2 * bt:], ir_target)
    return 0.5 * (rgb_loss + ir_loss)
...
def train(args, model: Model, dataset, *arg):
    ...
    use_p2r = (
        bool(getattr(args, 'enable_p2r', 0))
        and model.p2rnet is not None
        and not enable_phase1
        and epoch >= getattr(args, 'p2r_start_epoch', 60)
    )
    use_crccl = (
        bool(getattr(args, 'enable_crccl', 0))
        and not enable_phase1
        and epoch >= getattr(args, 'crccl_start_epoch', 20)
    )
    p2r_anchor_mask, p2r_candidate_mask = None, None
    crccl_prev_common_pairs = getattr(cma, 'crccl_prev_common_pairs', set())
    ...
    if 'wsl' in args.debug or not enable_phase1:
        ...
        if use_p2r:
            p2r_anchor_mask, p2r_candidate_mask, p2r_mask_stats = _build_p2r_masks(
                args, model, cma, epoch, common_dict, specific_dict, remain_dict,
                r2i_pair_dict, i2r_pair_dict
            )
            with torch.no_grad():
                p2r_epoch_pack = model.p2rnet(
                    cma.vis_memory.to(model.device),
                    cma.ir_memory.to(model.device),
                    p2r_anchor_mask,
                    p2r_candidate_mask,
                )
                p2r_diag = p2r_epoch_pack['diag']
            logger(
                'P2R diag | anchors:{} common:{} candidates:{}({:.2f}%) '
                'row_conf:{:.4f} col_conf:{:.4f} entropy:{:.4f} '
                'anchor_p:{:.4f} dustbin_r:{:.4f} dustbin_c:{:.4f}'.format(
                    p2r_mask_stats['anchors'],
                    p2r_mask_stats['common_pairs'],
                    p2r_mask_stats['candidates'],
                    p2r_mask_stats['candidate_ratio'] * 100,
                    float(p2r_diag['row_conf'].item()),
                    float(p2r_diag['col_conf'].item()),
                    float(p2r_diag['entropy'].item()),
                    float(p2r_diag['anchor_prob'].item()),
                    float(p2r_diag['row_dustbin'].item()),
                    float(p2r_diag['col_dustbin'].item()),
                )
            )
        else:
            cma.p2r_prev_common_pairs = _as_pair_set(common_dict)
        cma.crccl_prev_common_pairs = _as_pair_set(common_dict)
    ...
    for (rgb_imgs, ca_imgs, color_info), (ir_imgs, aug_imgs,ir_info) in zip(rgb_loader, ir_loader):
        ...
        if 'wsl' in args.debug:
            if enable_phase1:
                ...
                if args.debug == 'wsl':
                    ...
                    crccl_enabled = use_crccl and args.debug == 'wsl'
                    crccl_replace_cmo = crccl_enabled and bool(getattr(args, 'crccl_replace_cmo', 0))

                    # 验证并叠加 CMO 损失
                    if (selected_tri_ir_ids.shape[0]!=0) and not crccl_replace_cmo:
                    ...
                    if (selected_tri_rgb_ids.shape[0]!=0) and not crccl_replace_cmo:
                    ...
                    if crccl_enabled:
                        crccl_features = rgmfd_pack['shared_bn'] if rgmfd_pack is not None else bn_features
                        crccl_cm_weight = getattr(args, 'crccl_cm_weight', 0.0)
                        crccl_shared_weight = getattr(args, 'crccl_shared_weight', 0.0)
                        crccl_cf_weight = getattr(args, 'crccl_cf_weight', 0.0)
                        crccl_weights_for_meter = []

                        if selected_common_ir_ids.shape[0] != 0:
                            selected_ir_shared = crccl_features[2*bt:][common_ir_indices]
                            ir_weights = _crccl_relation_weights(
                                args,
                                i2i_cls[common_ir_indices],
                                mem_r2i_cls,
                                selected_ir_shared,
                                selected_rgb_memory,
                                translated_cmo_ir_label,
                                selected_common_ir_ids,
                                crccl_prev_common_pairs,
                            )
                            if crccl_cm_weight > 0:
                                crccl_ir_loss = (
                                    common_loss_weight
                                    * crccl_cm_weight
                                    * _weighted_bidirectional_kl(
                                        i2i_cls[common_ir_indices],
                                        mem_r2i_cls,
                                        ir_weights,
                                        temperature=getattr(args, 'crccl_temp', 3.0),
                                ...
                                if torch.isnan(crccl_ir_loss).any():
                                    nan_batch_counter += 1
                                else:
                                    meter.update({'crccl_ir_loss': crccl_ir_loss.data})
                                    total_loss += crccl_ir_loss
                            if crccl_shared_weight > 0:
                                crccl_ir_shared_loss = (
                                    common_loss_weight
                                    * crccl_shared_weight
                                ...
                                if torch.isnan(crccl_ir_shared_loss).any():
                                    nan_batch_counter += 1
                                else:
                                    meter.update({'crccl_ir_shared_loss': crccl_ir_shared_loss.data})
                                    total_loss += crccl_ir_shared_loss
                            if crccl_cf_weight > 0 and rgmfd_pack is not None:
                                selected_ir_specific = rgmfd_pack['specific_bn'][2*bt:][common_ir_indices]
                                crccl_ir_cf_loss = (
                                    common_loss_weight
                                    * crccl_cf_weight
                                    * _crccl_counterfactual_loss(
                                        args,
                                        model,
                                        selected_ir_shared,
                                        selected_ir_specific,
                                        i2c_cls[common_ir_indices],
                                        ir_weights,
                                    )
                                )
                                if torch.isnan(crccl_ir_cf_loss).any():
                                    nan_batch_counter += 1
                                else:
                                    meter.update({'crccl_ir_cf_loss': crccl_ir_cf_loss.data})
                                    total_loss += crccl_ir_cf_loss
                            crccl_weights_for_meter.append(ir_weights.mean())

                        if selected_common_rgb_ids.shape[0] != 0:
                            selected_rgb_shared = crccl_features[:2*bt][common_rgb_indices]
                            rgb_weights = _crccl_relation_weights(
                                args,
                                r2r_cls[common_rgb_indices],
                                mem_i2r_cls,
                                selected_rgb_shared,
                                selected_ir_memory,
                                selected_common_rgb_ids,
                            ...
                            if crccl_cm_weight > 0:
                                ...
                                if torch.isnan(crccl_rgb_loss).any():
                                    nan_batch_counter += 1
                                else:
                                    meter.update({'crccl_rgb_loss': crccl_rgb_loss.data})
                                    total_loss += crccl_rgb_loss
                            if crccl_shared_weight > 0:
                                crccl_rgb_shared_loss = (
                                    common_loss_weight
                                    * crccl_shared_weight
                                    * _weighted_cosine_alignment_loss(
                                        selected_rgb_shared,
                                ...
                                if torch.isnan(crccl_rgb_shared_loss).any():
                                    nan_batch_counter += 1
                                else:
                                    meter.update({'crccl_rgb_shared_loss': crccl_rgb_shared_loss.data})
                                    total_loss += crccl_rgb_shared_loss
                            if crccl_cf_weight > 0 and rgmfd_pack is not None:
                                selected_rgb_specific = rgmfd_pack['specific_bn'][:2*bt][common_rgb_indices]
                                crccl_rgb_cf_loss = (
                                    common_loss_weight
                                    * crccl_cf_weight
                                    * _crccl_counterfactual_loss(
                                        ...
                                        model,
                                        selected_rgb_shared,
                                        selected_rgb_specific,
                                        r2c_cls[common_rgb_indices],
                                        rgb_weights,
                                    )
                                )
                                if torch.isnan(crccl_rgb_cf_loss).any():
                                    nan_batch_counter += 1
                                else:
                                    meter.update({'crccl_rgb_cf_loss': crccl_rgb_cf_loss.data})
                                    total_loss += crccl_rgb_cf_loss
                            crccl_weights_for_meter.append(rgb_weights.mean())

                        if len(crccl_weights_for_meter) > 0:
                            meter.update({'crccl_weight': torch.stack(crccl_weights_for_meter).mean().data})
                    ...
                    if rgmfd_pack is not None and epoch >= getattr(args, 'rgmfd_start_epoch', 0):
                        ...
                        if selected_common_rgb_ids.shape[0] != 0:
                            ...
                            rel_losses.append(_cosine_alignment_loss(selected_rgb_shared, selected_ir_memory))
                        if selected_common_ir_ids.shape[0] != 0:
                            selected_ir_shared = shared_bn[2*bt:][common_ir_indices]
                            rel_losses.append(_cosine_alignment_loss(selected_ir_shared, selected_rgb_memory))
                        if len(rel_losses) > 0:
                            rgmfd_rel_loss = (
                                common_loss_weight
                ...
                if args.debug == 'wsl' and use_p2r:
                    p2r_pack = model.p2rnet(
                        cma.vis_memory.to(model.device),
                        cma.ir_memory.to(model.device),
                        p2r_anchor_mask,
                        p2r_candidate_mask,
                    )
                    p2r_anchor_weight = getattr(args, 'p2r_anchor_weight', 0.0)
                    if p2r_anchor_weight > 0:
                        p2r_anchor_loss = p2r_anchor_weight * _p2r_anchor_loss(p2r_pack, p2r_anchor_mask)
                        if not torch.isnan(p2r_anchor_loss).any():
                            meter.update({'p2r_anchor_loss': p2r_anchor_loss.data})
                            total_loss += p2r_anchor_loss
                    p2r_unc_weight = getattr(args, 'p2r_unc_weight', 0.0)
                    if p2r_unc_weight > 0:
                        p2r_unc_loss = p2r_unc_weight * _p2r_uncertainty_loss(p2r_pack, p2r_anchor_mask)
                        if not torch.isnan(p2r_unc_loss).any():
                            meter.update({'p2r_unc_loss': p2r_unc_loss.data})
                            total_loss += p2r_unc_loss
                    if epoch >= getattr(args, 'p2r_refine_start_epoch', 70):
                        refine_weight = getattr(args, 'p2r_refine_weight', 0.0)
                        if refine_weight > 0:
                            shared_features = rgmfd_pack['shared_bn'] if rgmfd_pack is not None else bn_features
                            p2r_refine_loss = refine_weight * _p2r_refine_loss(
                                p2r_pack, shared_features, rgb_ids, ir_ids, bt
                            )
                            if not torch.isnan(p2r_refine_loss).any():
                                meter.update({'p2r_refine_loss': p2r_refine_loss.data})
                                total_loss += p2r_refine_loss
                ...
                if epoch >= remain_start_epoch:
                    ...
                    if (
                        use_p2r
                        and p2r_pack is not None
                        and getattr(args, 'p2r_use_soft_remain', 1)
                    ):
                        soft_weight = remain_weight * getattr(args, 'p2r_soft_weight', 0.0)
                        p2r_remain_rgb, kept_rgb, conf_rgb = _p2r_soft_relation_ce(
                            remain_r2c_cls,
                            p2r_pack['relation'],
                            remain_rgb_ids,
                            direction='r2i',
                            topk=getattr(args, 'p2r_candidate_topk', 5),
                        )
                        p2r_remain_ir, kept_ir, conf_ir = _p2r_soft_relation_ce(
                            remain_i2c_cls,
                            p2r_pack['relation'],
                            remain_ir_ids,
                            direction='i2r',
                        ...
                        p2r_remain_loss = soft_weight * 0.5 * (p2r_remain_rgb + p2r_remain_ir)
                        if not torch.isnan(p2r_remain_loss).any() and (kept_rgb + kept_ir) > 0:
                            meter.update({
                                'p2r_remain_loss': p2r_remain_loss.data,
                                'p2r_remain_conf': 0.5 * (conf_rgb + conf_ir),
                            })
                            total_loss += p2r_remain_loss
                    else:
        ...
        if enable_phase1:
            ...
            if args.debug == 'wsl':
                ...
                if selected_rgb_ids.shape[0] > 0 or (
                    use_p2r
                    and p2r_pack is not None
                    and selected_ir_ids.shape[0] > 0
                    and getattr(args, 'p2r_use_soft_specific', 1)
                ):
                    ...
                    if (
                        ...
                        p2r_specific_rgb, kept_rgb, conf_rgb = _p2r_soft_relation_ce(
                            selected_r2c_cls,
                            p2r_pack['relation'],
                            selected_rgb_ids,
                            direction='r2i',
                        ...
                        p2r_specific_ir, kept_ir, conf_ir = _p2r_soft_relation_ce(
                            selected_i2c_cls,
                            p2r_pack['relation'],
                            selected_ir_ids,
                            direction='i2r',
                        ...
                        p2r_specific_loss = (
                            specific_weight
                            * getattr(args, 'p2r_soft_weight', 0.0)
                            * 0.5
                            * (p2r_specific_rgb + p2r_specific_ir)
                        )
                        if not torch.isnan(p2r_specific_loss).any() and (kept_rgb + kept_ir) > 0:
                            meter.update({
                                'p2r_specific_loss': p2r_specific_loss.data,
                                'p2r_specific_conf': 0.5 * (conf_rgb + conf_ir),
                            })
                            total_loss += p2r_specific_loss
                    else:
