...
class CMA(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.match_ema = getattr(args, 'match_ema', 0.8)
        self.match_high_conf = getattr(args, 'match_high_conf', 0.55)
        self.match_low_conf = getattr(args, 'match_low_conf', 0.35)
        self.match_stable_epochs = getattr(args, 'match_stable_epochs', 2)
        ...
        self.register_buffer('temporal_row_scores', torch.zeros(self.num_classes, self.num_classes))
        self.register_buffer('temporal_col_scores', torch.zeros(self.num_classes, self.num_classes))
        ...
        self.register_buffer('prev_row_assign', torch.full((self.num_classes,), -1, dtype=torch.long))
        self.register_buffer('prev_col_assign', torch.full((self.num_classes,), -1, dtype=torch.long))
        self.register_buffer('row_stability', torch.zeros(self.num_classes, dtype=torch.long))
        self.register_buffer('col_stability', torch.zeros(self.num_classes, dtype=torch.long))
        self.register_buffer('temporal_ready', torch.tensor(False, dtype=torch.bool))
    ...
    def get_label(self, epoch=None):
        if self.not_saved:# pass if 
            ...
            if self.mode == 'features':
                ...
                v2i_dict, i2v_dict = self._get_hybrid_label()
                self.v2i = v2i_dict
                self.i2v = i2v_dict
    ...
    def _get_hybrid_label(self):
        ...
        if not bool(self.temporal_ready.item()):
            temporal_row = vis_proto
            temporal_col = ir_proto
            self.temporal_ready.fill_(True)
        else:
            temporal_row = self.match_ema * self.temporal_row_scores.to(self.device) + (1 - self.match_ema) * vis_proto
            temporal_col = self.match_ema * self.temporal_col_scores.to(self.device) + (1 - self.match_ema) * ir_proto

        row_pairs, _ = self._get_label(temporal_row.detach().cpu().numpy(), 'rgb')
        col_pairs, _ = self._get_label(temporal_col.detach().cpu().numpy(), 'ir')
        ...
        row_assign = torch.full((self.num_classes,), -1, dtype=torch.long, device=self.device)
        col_assign = torch.full((self.num_classes,), -1, dtype=torch.long, device=self.device)
        for r, i in row_pairs.items():
            row_assign[r] = i
        for i, r in col_pairs.items():
            col_assign[i] = r

        prev_row_assign = self.prev_row_assign.to(self.device)
        prev_col_assign = self.prev_col_assign.to(self.device)
        prev_row_stability = self.row_stability.to(self.device)
        prev_col_stability = self.col_stability.to(self.device)
        row_stability = torch.where(
            row_assign >= 0,
            torch.where(row_assign == prev_row_assign, prev_row_stability + 1, torch.ones_like(prev_row_stability)),
            torch.zeros_like(prev_row_stability)
        )
        col_stability = torch.where(
            col_assign >= 0,
            torch.where(col_assign == prev_col_assign, prev_col_stability + 1, torch.ones_like(prev_col_stability)),
            torch.zeros_like(prev_col_stability)
        )

        common_dict, specific_dict, remain_dict = {}, {}, {}
        for r, i in row_pairs.items():
            pair_conf = min(float(row_conf[r].item()), float(col_conf[i].item()))
            pair_stability = min(int(row_stability[r].item()), int(col_stability[i].item()))
            if i in col_pairs.keys() and col_pairs[i] == r and \
               pair_conf >= self.match_high_conf and pair_stability >= self.match_stable_epochs:
                common_dict[r] = i
            elif r not in col_pairs.values() and i not in col_pairs.keys() and \
                 row_conf[r].item() >= self.match_low_conf and row_stability[r].item() >= self.match_stable_epochs:
                specific_dict[r] = i
            else:
                remain_dict[r] = i
        for i, r in col_pairs.items():
            if (r, i) in common_dict.items():
                continue
            elif r not in row_pairs.values() and i not in row_pairs.keys() and \
                 col_conf[i].item() >= self.match_low_conf and col_stability[i].item() >= self.match_stable_epochs:
                specific_dict[r] = i
            else:
                remain_dict[r] = i

        common_row_mask = torch.zeros(self.num_classes, dtype=torch.bool, device=self.device)
        common_col_mask = torch.zeros(self.num_classes, dtype=torch.bool, device=self.device)
        specific_row_mask = torch.zeros(self.num_classes, dtype=torch.bool, device=self.device)
        specific_col_mask = torch.zeros(self.num_classes, dtype=torch.bool, device=self.device)
        remain_row_mask = torch.zeros(self.num_classes, dtype=torch.bool, device=self.device)
        remain_col_mask = torch.zeros(self.num_classes, dtype=torch.bool, device=self.device)

        common_rm = torch.zeros_like(vis_proto)
        specific_rm = torch.zeros_like(vis_proto)
        specific_col_rm = torch.zeros_like(ir_proto)
        remain_rm = torch.zeros_like(vis_proto)
        remain_col_rm = torch.zeros_like(ir_proto)

        for r, i in common_dict.items():
            common_row_mask[r] = True
            common_col_mask[i] = True
            common_rm[r, i] = 1.0

        for r, i in specific_dict.items():
            specific_row_mask[r] = True
            specific_col_mask[i] = True
            specific_rm[r, i] = 1.0
            specific_col_rm[i, r] = 1.0

        for r, i in remain_dict.items():
            remain_row_mask[r] = True
            remain_col_mask[i] = True
            remain_rm[r, i] = 1.0
            remain_col_rm[i, r] = 1.0

        specific_rm = specific_rm + common_rm
        specific_col_rm = specific_col_rm + common_rm.t()

        self.row_scores.copy_(temporal_row)
        self.col_scores.copy_(temporal_col)
        self.temporal_row_scores.copy_(temporal_row)
        self.temporal_col_scores.copy_(temporal_col)
        ...
        self.prev_row_assign.copy_(row_assign)
        self.prev_col_assign.copy_(col_assign)
        self.row_stability.copy_(row_stability)
        self.col_stability.copy_(col_stability)
