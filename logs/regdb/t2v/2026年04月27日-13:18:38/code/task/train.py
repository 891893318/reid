        ...
        if 'wsl' in args.debug:
            if enable_phase1:
                ...
                if args.phase2_pseudo_warmup > 0:
                    ...
                    ramp = min(float(epoch + 1) / float(args.phase2_pseudo_warmup), 1.0)
                    pseudo_weight = start_weight + (1.0 - start_weight) * ramp
                ...
                if args.debug == 'wsl':
                    ...
                    if (selected_tri_ir_ids.shape[0]!=0):
                        r2i_cmo_loss = pseudo_weight * w_r2i * cmo_criterion(dtd_i2i_cls[common_ir_indices],mem_r2i_cls)
                    ...
                    if (selected_tri_rgb_ids.shape[0]!=0):
                        i2r_cmo_loss = pseudo_weight * w_i2r * cmo_criterion(dtd_r2r_cls[common_rgb_indices],mem_i2r_cls)
