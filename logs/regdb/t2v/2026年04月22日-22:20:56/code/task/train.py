...
def train(args, model: Model, dataset, *arg):
    ...
    if 'wsl' in args.debug or not enable_phase1:
        ...
        transport_rgb = cma.transport_row.to(model.device)
        transport_ir = cma.transport_col.to(model.device)
    ...
    for (rgb_imgs, ca_imgs, color_info), (ir_imgs, aug_imgs,ir_info) in zip(rgb_loader, ir_loader):
        ...
        if 'wsl' in args.debug:
            if enable_phase1:
                ...
                rgb_transport_targets = transport_rgb[rgb_ids]
                ir_transport_targets = transport_ir[ir_ids]
                rgb_transport_weights = row_conf[rgb_ids]
                ir_transport_weights = col_conf[ir_ids]
                ...
                if args.debug == 'wsl':
                    # ================ A. BCPT 软传输监督 ================
                    rgb_bcpt_loss = weighted_soft_ce(r2c_cls, rgb_transport_targets, rgb_transport_weights)
                    ir_bcpt_loss = weighted_soft_ce(i2c_cls, ir_transport_targets, ir_transport_weights)
                    meter.update({'rgb_bcpt_loss': rgb_bcpt_loss.data,
                                  'ir_bcpt_loss': ir_bcpt_loss.data})
                    total_loss += rgb_bcpt_loss + ir_bcpt_loss
                    ...
                    if selected_common_rgb_ids.shape[0] != 0:
                        transported_ir_memory = torch.matmul(
                            transport_rgb[selected_common_rgb_ids],
                            cma.ir_memory.detach()
                        )
                        mem_r2i_cls,_ = model.classifier2(transported_ir_memory)
                        r2i_cmo_loss = weighted_mse(
                            dtd_r2i_cls[common_rgb_indices],
                            mem_r2i_cls,
                    ...
                    if selected_common_ir_ids.shape[0] != 0:
                        transported_rgb_memory = torch.matmul(
                            transport_ir[selected_common_ir_ids],
                            cma.vis_memory.detach()
                        )
                        mem_i2r_cls,_ = model.classifier1(transported_rgb_memory)
                        i2r_cmo_loss = weighted_mse(
                            dtd_i2r_cls[common_ir_indices],
                            mem_i2r_cls,
                ...
                if epoch >= 30:
                    ...
                    if remain_ir_ids.shape[0] > 0:
                        weak_i2c_loss = args.weak_weight*model.weak_criterion(remain_i2c_cls, remain_col_rm[remain_ir_ids])
        ...
        if enable_phase1:
            ...
            if args.debug == 'wsl':
                ...
                selected_i2c_cls = i2c_cls[specific_ir_indices]
                selected_r2c_cls = r2c_cls[specific_rgb_indices]
