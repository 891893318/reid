...
def train(args, model: Model, dataset, *arg):
    ...
    if 'wsl' in args.debug or not enable_phase1:
        ...
        cma.extract(
            ...
            model,
            dataset,
            enable_graymix=bool(getattr(args, 'enable_graymix_audit', 0)) and not enable_phase1,
        )
        ...
        remain_dict, graymix_stats = cma.audit_remain_relations(remain_dict, epoch)
        if graymix_stats['enabled']:
            logger(
                'GrayMix audit | gm_r2i:{:.2f}%({}/{}) remain_keep:{}/{}({:.2f}%) '
                'remain_acc:{:.2f}%->{:.2f}%'.format(
                    graymix_stats['graymix']['acc'] * 100,
                    graymix_stats['graymix']['correct'],
                    graymix_stats['graymix']['valid'],
                    graymix_stats['kept'],
                    graymix_stats['total'],
                    graymix_stats['keep_ratio'] * 100,
                    graymix_stats['before']['acc'] * 100,
                    graymix_stats['after']['acc'] * 100,
                )
            )
