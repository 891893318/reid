...
class RegDB:
    def __init__(self, args):
        ...
        self.args.num_cams = max(self.train_rgb.num_cams, self.train_ir.num_cams)
...
class RegDB_train(data.Dataset):
    ...
    def _init_data(self):
        ...
        img_file, train_label, train_cam = self._load_data(train_list)
        train_image = []
        ...
        train_cam = np.array(train_cam).reshape(length,1)
        ...
        self.num_cams = int(train_cam.max()) + 1 if len(train_cam) > 0 else 1
    ...
    def _load_data(self,input_data_path):
        with open(input_data_path) as f:
            ...
            modal_cam = 0 if self.modal == 'rgb' else 1
            file_camid = [modal_cam for _ in file_image]
