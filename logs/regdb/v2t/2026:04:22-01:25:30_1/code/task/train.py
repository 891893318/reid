...
def train(args, model: Model, dataset, *arg):
    ...
    for (rgb_imgs, ca_imgs, color_info), (ir_imgs, aug_imgs,ir_info) in zip(rgb_loader, ir_loader):
        ...
        if model.should_apply_molora(enable_phase1):
            bn_features = model.adapt_paired_features(bn_features, 2 * bt)
        ...
        if enable_phase1:
            total_loss.backward()
            if model.enable_molora and model.molora_phase1 and model.molora_grad_clip > 0:
                torch.nn.utils.clip_grad_norm_(model.molora.parameters(), model.molora_grad_clip)
            ...
            if model.enable_molora and not enable_phase1:
                molora_align_loss = args.molora_align_weight * model.molora_loss(
                    bn_features[:2*bt],
                    bn_features[2*bt:],
                    rgb_ids,
                    ir_ids,
                )
                total_loss += molora_align_loss
                meter.update({'molora_align_loss': molora_align_loss.data})
        
            # Phase 2 主优化器梯度的向后传播更新
            total_loss.backward()
            if model.enable_molora and model.molora_grad_clip > 0:
                torch.nn.utils.clip_grad_norm_(model.molora.parameters(), model.molora_grad_clip)
